...
function downloadDemoExport() {
  ...
  link.href = url;
  link.download = "le-yard-os-demo-export.json";
  link.click();
...
function OrganizationPanel() {
  ...
  return (
    <div className="space-y-9">
      <section>
        <SectionHeading title="Organization profile" detail="This owner playground stays isolated from any future production tenant." />
      ...
      <section>
        <SectionHeading title="Owner accounts" detail="Production Owners require MFA. These two temporary preview identities use standard eight-hour sessions; existing passwords are never retrievable." />
        <div className="border-y border-[var(--line)]">
          {owners.map((owner, index) => (
            <div key={owner.id} className="flex items-center gap-3 border-t border-[var(--line)] py-4 first:border-0">
              ...
              <div className="min-w-0 flex-1"><p className="text-xs font-semibold">{owner.displayName}</p><p className="mt-1 text-[9px] text-[var(--ink-faint)]">Temporary playground identity · Owner operator</p></div>
              <StatusPill tone="accent"><ShieldCheck className="size-3" /> Preview owner</StatusPill>
            </div>
          ))}
        </div>
      </section>
    ...
    <div className="space-y-9">
      <section>
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-[16px] bg-[var(--canvas)] p-4"><p className="text-[9px] text-[var(--ink-faint)]">Synthetic fixtures</p><p className="numeric mt-2 text-xs font-semibold">{mfaEnabled} of {demoWorkspace.memberships.length} marked</p></div>
      ...
      <section>
        ...
        <div className="border-y border-[var(--line)]">
          {[
            ...
            [FileClock, "Session history", "Preview-only session · provider logging comes later"],
    ...
    <div className="space-y-9">
      <section>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-[18px] border border-[var(--line)] p-4"><DatabaseBackup className="size-4 text-[var(--ink-soft)]" /><p className="mt-4 text-xs font-semibold">Database backups</p><p className="mt-1 text-[9px] text-[var(--ink-faint)]">Production project pending</p><StatusPill className="mt-3">Not active</StatusPill></div>
      ...
      <section>
        <SectionHeading title="Tenant export" detail="Production exports are generated server-side, permission-checked, and written to the audit log." action={<Button variant="secondary" size="sm" onClick={downloadDemoExport}><Download className="size-3.5" /> Export demo JSON</Button>} />
        <div className="flex items-start gap-3 rounded-[16px] bg-[var(--positive-soft)] p-4 text-[10px] leading-4 text-[var(--positive)]"><ShieldCheck className="mt-0.5 size-4 shrink-0" /><span>This playground export contains synthetic demo records only. Cross-tenant exports are denied by membership checks and database RLS.</span></div>
      </section>
  ...
  return (
    <PageFrame width="standard">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div><p className="eyebrow">Le Yard playground</p><h2 className="mt-3 text-2xl font-medium tracking-[-0.045em]">Settings</h2><p className="mt-1 text-[11px] text-[var(--ink-faint)]">Tenant identity, locations, owner draft assumptions, security, and data controls</p></div>
        <StatusPill tone="accent"><CircleAlert className="size-3" /> Nonproduction playground</StatusPill>
      </div>
