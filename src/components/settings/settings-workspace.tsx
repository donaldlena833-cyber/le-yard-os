  ...
  return (
    <div className="space-y-9">
      <section>
        ...
        <p className="mt-3 flex items-start gap-2 text-xs leading-4 text-[var(--ink-faint)]"><CircleAlert className="mt-0.5 size-3.5 shrink-0" />Donald, Maris, Irini, and Mateo are the current Le Yard accounts. Operational records remain empty until your team adds them.</p>
      </section>
        ...
        <div className="border-y border-[var(--line)]">
          {owners.map((owner, index) => (
            <div key={owner.id} className="flex items-center gap-3 border-t border-[var(--line)] py-4 first:border-0">
              ...
              <div className="min-w-0 flex-1"><p className="text-xs font-semibold">{owner.displayName}</p><p className="mt-1 text-xs text-[var(--ink-faint)]">Owner operator · authenticated account</p></div>
      ...
      <section>
        ...
        <div className="overflow-x-auto border-y border-[var(--line)]">
          <div className="grid min-w-[620px] grid-cols-[110px_1fr_70px] gap-4 bg-[var(--canvas-strong)] px-4 py-2.5 text-xs font-semibold tracking-[.12em] text-[var(--ink-faint)] uppercase"><span>Role</span><span>Maximum scope</span><span>People</span></div>
          {roleSummary.map((item) => <div key={item.role} className="grid min-w-[620px] grid-cols-[110px_1fr_70px] items-center gap-4 border-t border-[var(--line)] px-4 py-3.5"><span className="text-xs font-semibold capitalize">{item.role}</span><span className="text-xs text-[var(--ink-faint)]">{item.scope}</span><span className="numeric text-xs">{item.people}</span></div>)}
        </div>
      </section>
    </div>
  );
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {demoWorkspace.locations.slice(0, 1).map((location, index) => (
          <article key={location.id} className="rounded-[20px] border border-[var(--line)] bg-[var(--paper)] p-5">
            ...
            <div className="mt-5 flex items-center justify-between border-t border-[var(--line)] pt-4"><span className="text-xs text-[var(--ink-faint)]">{location.timezone}</span><Button variant="quiet" size="sm">Review <ChevronRight className="size-3" /></Button></div>
          </article>
        ))}
      </div>
      <p className="mt-5 flex items-start gap-2 rounded-[16px] bg-[var(--warning-soft)] p-4 text-xs leading-4 text-[var(--warning)]"><CircleAlert className="mt-0.5 size-4 shrink-0" />Le Yard is currently configured as one main dining room. Phone, service periods, and job codes remain mock values for testing.</p>
    </div>
  );
}
...
function DraftRuleRow({
  ...
  return (
    <div className="group grid gap-3 border-t border-[var(--line)] px-1 py-4 first:border-0 sm:grid-cols-[38px_minmax(130px,.55fr)_minmax(220px,1fr)_auto] sm:items-center sm:gap-4">
      ...
      <div><p className="text-xs font-semibold tracking-[.12em] text-[var(--ink-faint)] uppercase">{title}</p><p className="mt-1 text-sm font-semibold tracking-[-0.025em] text-[var(--ink)]">{value}</p></div>
      <p className="text-xs leading-[1.55] text-[var(--ink-faint)]">{detail}</p>
      ...
      <section className="overflow-hidden rounded-[22px] bg-[var(--ink)] px-5 py-6 text-[var(--paper)] sm:px-7 sm:py-7">
        <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-start">
          <div className="max-w-xl">
            <p className="text-xs font-semibold tracking-[.16em] text-white/45 uppercase">Owner draft · unpublished</p>
            <h3 className="mt-3 text-2xl font-medium tracking-[-0.045em]">Reference only.</h3>
            <p className="mt-2 text-[13px] leading-5 text-white/58">These assumptions capture the owners’ current thinking for the playground. They do not claim legal compliance and cannot calculate schedules, payroll, tips, fees, or deletion dates.</p>
          </div>
    ...
    <div className="space-y-9">
      <section>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="rounded-[18px] border border-[var(--line)] p-4"><Archive className="size-4 text-[var(--warning)]" /><p className="mt-4 text-xs font-semibold">Retention policy</p><p className="mt-1 text-xs text-[var(--ink-faint)]">Not configured</p><StatusPill className="mt-3" tone="warning">Owner decision</StatusPill></div>
          <div className="rounded-[18px] border border-[var(--line)] p-4"><DatabaseBackup className="size-4 text-[var(--ink-soft)]" /><p className="mt-4 text-xs font-semibold">Database backups</p><p className="mt-1 text-xs text-[var(--ink-faint)]">Supabase-managed</p><StatusPill className="mt-3">Available</StatusPill></div>
          <div className="rounded-[18px] border border-[var(--line)] p-4"><MonitorCheck className="size-4 text-[var(--ink-soft)]" /><p className="mt-4 text-xs font-semibold">Error monitoring</p><p className="mt-1 text-xs text-[var(--ink-faint)]">Schema and boundaries ready</p><StatusPill className="mt-3">Needs destination</StatusPill></div>
        </div>
      </section>
      ...
      <section>
        ...
        <div className="flex items-start gap-3 rounded-[16px] bg-[var(--positive-soft)] p-4 text-xs leading-4 text-[var(--positive)]"><ShieldCheck className="mt-0.5 size-4 shrink-0" /><span>Exports are tenant-scoped. Cross-tenant records are denied by membership checks and database RLS.</span></div>
      </section>
      ...
      <section>
        ...
        <div className="overflow-x-auto border-y border-[var(--line)]">
          <div className="grid min-w-[700px] grid-cols-[1fr_1fr_.8fr_.7fr] gap-4 bg-[var(--canvas-strong)] px-4 py-2.5 text-xs font-semibold tracking-[.12em] text-[var(--ink-faint)] uppercase"><span>Event</span><span>Record</span><span>Actor</span><span>Occurred</span></div>
          {events.map((event) => <div key={event.id} className="grid min-w-[700px] grid-cols-[1fr_1fr_.8fr_.7fr] items-center gap-4 border-t border-[var(--line)] px-4 py-3.5"><span className="text-xs font-semibold">{event.action.replaceAll("_", " ")}</span><span className="font-mono text-xs text-[var(--ink-faint)]">{event.entityType} · {event.entityId}</span><span className="text-xs capitalize text-[var(--ink-faint)]">{event.actorType}</span><span className="numeric text-xs text-[var(--ink-faint)]">{new Date(event.occurredAt).toLocaleString()}</span></div>)}
        </div>
      </section>
    </div>
  );
  ...
  return (
    <PageFrame width="standard">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div><p className="eyebrow">Le Yard playground</p><h2 className="mt-3 text-2xl font-medium tracking-[-0.045em]">Settings</h2><p className="mt-1 text-[13px] text-[var(--ink-faint)]">Staff profiles, job codes, schedules, receipts, and operational records start empty here. synthetic mock data is off.</p></div>
