...
type Tab = "organization" | "locations" | "capabilities" | "expenses" | "security" | "notifications" | "data";
  ...
  return <div className="space-y-9">
    <section>
      ...
      <dl className="grid gap-px overflow-hidden rounded-[18px] border border-[var(--line)] bg-[var(--line)] sm:grid-cols-2">
        ...
        ].map(([label, value]) => <div key={label} className="bg-[var(--paper)] px-4 py-4"><dt className="text-xs font-semibold tracking-[.1em] text-[var(--ink-faint)] uppercase">{label}</dt><dd className="mt-2 text-xs font-semibold">{value}</dd></div>)}
      </dl>
      <div className={cn("mt-4 flex items-start gap-3 rounded-[16px] p-4 text-xs leading-4", data.organization.configuredAt ? "bg-[var(--positive-soft)] text-[var(--positive)]" : "bg-[var(--warning-soft)] text-[var(--warning)]")}><CircleAlert className="mt-0.5 size-4 shrink-0" /><span>{data.organization.configuredAt ? `Tenant setup was marked configured ${dateTime(data.organization.configuredAt)}. Owner approval is still required before production deployment.` : "Owner emails, exact restaurant and location details, brand assets, and operating rules are still required before production bootstrap."}</span></div>
    </section>
    <section>
      <SectionHeading title="Owner accounts" detail="Owners have full tenant access. Auth email and password custody stay with Supabase Auth." />
      <div className="border-y border-[var(--line)]">{data.owners.map((owner, index) => <div key={owner.userId} className="flex items-center gap-3 border-t border-[var(--line)] py-4 first:border-0"><Avatar name={owner.displayName} index={index} /><div className="min-w-0 flex-1"><p className="text-xs font-semibold">{owner.displayName}</p><p className="mt-1 text-xs text-[var(--ink-faint)]">Owner operator · password not retrievable</p></div><StatusPill tone={tone(owner.status)}><ShieldCheck className="size-3" /> {owner.status}</StatusPill></div>)}{!data.owners.length ? <p className="py-5 text-center text-xs text-[var(--ink-faint)]">No owner account is visible in this tenant scope.</p> : null}</div>
    </section>
    <section>
...
function NotificationsPanel({ data, workspace }: { data: LiveSettingsModel; workspace: WorkspaceContextValue }) {
  ...
  return <div className="space-y-9"><section><SectionHeading title="In-app alerts" detail="Choose which server-derived events appear in your notification center." /><div className="border-y border-[var(--line)]">{notificationCatalog.map(([type, label, detail]) => <div key={type} className="grid gap-3 border-t border-[var(--line)] py-4 first:border-0 sm:grid-cols-[minmax(0,1fr)_auto_auto_auto] sm:items-center"><div><p className="text-xs font-semibold">{label}</p><p className="mt-1 text-xs text-[var(--ink-faint)]">{detail}</p></div><button type="button" role="switch" aria-checked={localInApp[type]} disabled={pending} onClick={() => saveInApp(type, !localInApp[type])} className={cn("focus-ring flex h-8 items-center gap-2 rounded-full px-2.5 text-xs font-semibold", localInApp[type] ? "bg-[var(--positive-soft)] text-[var(--positive)]" : "bg-[var(--canvas-strong)] text-[var(--ink-faint)]")}><span className={cn("size-2 rounded-full", localInApp[type] ? "bg-[var(--positive)]" : "bg-[var(--ink-faint)]")} />{localInApp[type] ? "In app on" : "In app off"}</button><StatusPill tone="neutral"><Mail className="size-3" />Email inactive</StatusPill><StatusPill tone="neutral"><Smartphone className="size-3" />Push inactive</StatusPill></div>)}</div><p className="mt-3 text-xs leading-4 text-[var(--ink-faint)]">Email and push delivery controls remain inactive because no approved SMTP or push-delivery process is running. This screen does not claim those messages are sent.</p></section><section><SectionHeading title="Browser subscription storage" detail="Encrypted endpoint custody for a future approved push-delivery process." action={<Button variant="secondary" size="sm" disabled={pending || !vapidPublicKey} onClick={registerBrowser}>{pending ? <LoaderCircle className="size-3.5 animate-spin" /> : <Smartphone className="size-3.5" />}Register this browser</Button>} /><div className="border-y border-[var(--line)]">{data.pushSubscriptions.map((subscription) => <div key={subscription.id} className="flex items-center gap-3 border-t border-[var(--line)] py-4 first:border-0"><span className="flex size-8 items-center justify-center rounded-xl bg-[var(--canvas-strong)]"><Smartphone className="size-3.5 text-[var(--ink-soft)]" /></span><div className="min-w-0 flex-1"><p className="text-xs font-semibold">{subscription.deviceLabel ?? "Stored browser"}</p><p className="mt-1 font-mono text-xs text-[var(--ink-faint)]">Endpoint {subscription.endpointHash.slice(0, 12)}… · stored {dateTime(subscription.createdAt)}</p></div><Button variant="quiet" size="sm" disabled={pending} onClick={() => removeBrowser(subscription.endpointHash)}>Remove</Button></div>)}{!data.pushSubscriptions.length ? <p className="py-5 text-center text-xs text-[var(--ink-faint)]">No encrypted browser subscription is stored for your account.</p> : null}</div><div className="mt-4 flex items-start gap-3 rounded-[16px] bg-[var(--warning-soft)] p-4 text-xs leading-4 text-[var(--warning)]"><BellRing className="mt-0.5 size-4 shrink-0" /><span>Registering a browser stores encrypted subscription evidence only. It does not activate push delivery. VAPID signing keys and an approved delivery worker are still required.</span></div></section>{message ? <p role="status" aria-live="polite" className="rounded-xl bg-[var(--canvas)] px-4 py-3 text-xs">{message}</p> : null}</div>;
}
...
export function LiveSettingsWorkspace({ workspace, result }: { workspace: WorkspaceContextValue; result: LiveReadResult<LiveSettingsModel> }) {
  ...
  return <PageFrame width="standard"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="eyebrow">Tenant administration</p><h2 className="mt-3 text-2xl font-medium tracking-[-0.045em]">Settings</h2><p className="mt-1 text-[13px] text-[var(--ink-faint)]">Organization, locations, job-role capabilities, security, notifications, retention, and audit</p></div><StatusPill tone={data.organization.configuredAt ? "positive" : "warning"}><CircleAlert className="size-3" /> {data.organization.configuredAt ? "Tenant configured" : "Production setup incomplete"}</StatusPill></div><section className="mt-5 grid grid-cols-2 divide-x divide-y divide-[var(--line)] border-y border-[var(--line)] sm:grid-cols-4 sm:divide-y-0"><Metric label="Owners" value={String(data.roleCounts.owner)} detail={`${data.membershipCounts.active} active members`} /><Metric label="Locations" value={String(data.locations.length)} detail="Current access scope" /><Metric label="Session" value={workspace.identity.aal.toUpperCase()} detail={`${workspace.role} access`} /><Metric label="Retention" value={data.retentionPolicies.length ? "Recorded" : "Unset"} detail={data.retentionPolicies.length ? `${data.retentionPolicies.length} classes` : "Owner policy required"} /></section><div className="mt-8 grid gap-8 lg:grid-cols-[210px_1fr]"><nav aria-label="Settings sections" className="flex gap-1 overflow-x-auto lg:block lg:space-y-1">{tabs.map((item) => { const Icon = item.icon; const active = tab === item.id; return <button key={item.id} onClick={() => setTab(item.id)} aria-current={active ? "page" : undefined} className={cn("focus-ring flex min-h-10 shrink-0 items-center gap-2.5 rounded-xl px-3 text-left text-[13px] font-semibold transition-colors lg:w-full", active ? "bg-[var(--ink)] text-[var(--paper)]" : "text-[var(--ink-faint)] hover:bg-[var(--canvas-strong)] hover:text-[var(--ink)]")}><Icon className="size-3.5" />{item.label}</button>; })}</nav><div className="min-w-0">{tab === "organization" ? <OrganizationPanel data={data} /> : tab === "locations" ? <LocationsPanel data={data} /> : tab === "capabilities" ? <CapabilityConfiguration data={data} workspace={workspace} /> : tab === "expenses" ? <ExpenseCategoriesPanel data={data} workspace={workspace} /> : tab === "security" ? <SecurityPanel workspace={workspace} /> : tab === "notifications" ? <NotificationsPanel data={data} workspace={workspace} /> : <DataPanel data={data} workspace={workspace} />}</div></div></PageFrame>;
}
