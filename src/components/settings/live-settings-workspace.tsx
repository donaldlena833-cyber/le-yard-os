...
function tone(status: string): StatusTone {
  ...
  if (["queued", "running", "invited", "partially_succeeded"].includes(status))
    return "warning";
  return "neutral";
}
        ...
        <dl className="grid gap-px overflow-hidden rounded-[18px] border border-[var(--line)] bg-[var(--line)] sm:grid-cols-2">
          {[
            ...
            <div key={label} className="bg-[var(--paper)] px-4 py-4">
              ...
              <dd className="mt-2 text-xs font-semibold">{value}</dd>
            </div>
          ))}
        </dl>
        <div
          className={cn(
            "mt-4 flex items-start gap-3 rounded-[16px] p-4 text-xs leading-4",
      ...
      <section>
        ...
        <div className="border-y border-[var(--line)]">
          {data.owners.map((owner, index) => (
            <div
              key={owner.userId}
              className="flex items-center gap-3 border-t border-[var(--line)] py-4 first:border-0"
            >
              ...
              <StatusPill tone={tone(owner.status)}>
                <ShieldCheck className="size-3" /> {owner.status}
              </StatusPill>
            </div>
          ))}
          {!data.owners.length ? (
      ...
      <section>
        ...
        <div className="overflow-x-auto border-y border-[var(--line)]">
          ...
          {roles.map((role) => (
            <div
              key={role}
              className="grid min-w-[620px] grid-cols-[110px_1fr_70px] items-center gap-4 border-t border-[var(--line)] px-4 py-3.5"
            >
              <span className="text-xs font-semibold capitalize">{role}</span>
              <span className="text-xs text-[var(--ink-faint)]">
                {roleScope[role]}
              </span>
              <span className="numeric text-xs">{data.roleCounts[role]}</span>
            </div>
          ))}
        </div>
      </section>
...
function LocationsPanel({ data }: { data: LiveSettingsModel }) {
  return (
    <div>
        ...
        {data.locations.map((location, index) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <StatusPill tone={location.active ? "positive" : "neutral"} dot>
            ...
            <p className="mt-1 text-xs font-semibold tracking-[.1em] text-[var(--ink-faint)] uppercase">
              {location.code}
            </p>
            <p className="mt-3 min-h-8 text-xs leading-4 text-[var(--ink-faint)]">
      ...
      {data.canManage ? (
        <p className="mt-5 flex items-start gap-2 rounded-[16px] bg-[var(--warning-soft)] p-4 text-xs leading-4 text-[var(--warning)]">
          <LockKeyhole className="mt-0.5 size-4 shrink-0" />
          Location edits are intentionally locked until the owners supply and
          approve exact operating details.
        </p>
      ) : null}
    </div>
  );
    ...
    <div className="space-y-9">
      <section>
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-[16px] bg-[var(--canvas)] p-4">
            <p className="text-xs text-[var(--ink-faint)]">Session assurance</p>
            <p className="mt-2 text-xs font-semibold uppercase">
              {workspace.identity.aal}
            </p>
          </div>
          <div className="rounded-[16px] bg-[var(--canvas)] p-4">
            <p className="text-xs text-[var(--ink-faint)]">Owner requirement</p>
      ...
      <section>
        <SectionHeading
          ...
          detail="User creation and role assignment are restricted to Owner and Admin accounts."
        />
        <div className="border-y border-[var(--line)]">
          {[
            [
              UsersRound,
              "User creation",
              "One-time invitation · Owner or Admin",
            ],
            [
              LockKeyhole,
              "Password custody",
              "User-controlled · not recoverable by admins",
            ],
            [
              KeyRound,
              "Service credentials",
            ...
            return (
              ...
              >
                <span className="flex size-8 items-center justify-center rounded-xl bg-[var(--canvas-strong)]">
                  <RowIcon className="size-3.5 text-[var(--ink-soft)]" />
                </span>
                <div className="min-w-0 flex-1">
...
const notificationCatalog = [
  [
    "schedule_published",
    "Published schedules",
    "A new schedule version is published.",
  ],
  [
    "shift_assigned",
    "Shift assignments",
    "A published shift is assigned to you.",
  ],
  [
    "shift_swap_decided",
    "Shift swap decisions",
    "A requested shift swap is approved or declined.",
  ],
  [
    "time_correction_decided",
    "Time correction decisions",
    "A time-entry correction is approved or declined.",
  ],
  [
    "time_off_decided",
    "Time-off decisions",
    "A time-off request is approved or declined.",
  ],
  [
    "task_assigned",
    "Task assignments",
    "A new operations task is assigned to you.",
  ],
  [
    "reservation_changed",
    "Reservation changes",
    "An online reservation is confirmed, modified, or cancelled.",
  ],
] as const;
...
function NotificationsPanel({
  ...
  const [localInApp, setLocalInApp] = useState<Record<string, boolean>>(() =>
    Object.fromEntries(
      notificationCatalog.map(([type]) => [
        type,
        data.notificationPreferences.find(
  ...
  const reservationPushEnabled =
    data.notificationPreferences.find(
      (preference) => preference.notificationType === "reservation_changed",
    )?.push ?? false;

  function saveInApp(
    notificationType: (typeof notificationCatalog)[number][0],
    inApp: boolean,
  ) {
    setMessage(null);
    startTransition(async () => {
      const currentPreference = data.notificationPreferences.find(
      ...
      const payload = {
        ...
        inApp,
        email: false,
        push: currentPreference?.push ?? false,
        quietHours: {},
      };
      const result = await setNotificationPreferenceAction({
        requestId: requestIdFor(
          `notification.preference:${notificationType}`,
          payload,
        ),
        ...payload,
      });
      ...
      setMessage(
        "In-app preference saved. Future derived alerts will follow this setting.",
      );
      router.refresh();
    });
  }
  ...
  function registerBrowser() {
    ...
    startTransition(async () => {
      if (
        ...
        setMessage(
          "Browser subscription storage is unavailable in this deployment or browser.",
        );
        return;
      }
      ...
      const preferencePayload = {
        organizationId: workspace.organization.id,
        notificationType: "reservation_changed" as const,
        inApp: true,
        email: false,
        push: true,
        quietHours: {},
      };
      const preferenceResult = await setNotificationPreferenceAction({
        requestId: requestIdFor(
          "notification.preference:reservation_changed:push",
          preferencePayload,
        ),
        ...preferencePayload,
      });
      if (!preferenceResult.ok) {
        setMessage(
          `Browser stored, but reservation push could not be enabled: ${preferenceResult.message}`,
        );
        return;
      }
      rotateRequestId("notification.push.save");
      rotateRequestId("notification.preference:reservation_changed:push");
      setMessage(
        "This browser is ready for reservation-change push alerts when staff push is enabled for the location.",
      );
      router.refresh();
    });
  }
  ...
  function removeBrowser(hash: string) {
    ...
    startTransition(async () => {
      ...
      if ("serviceWorker" in navigator) {
        ...
        if (current && (await endpointHash(current.endpoint)) === hash)
  ...
  return (
    <div className="space-y-9">
      <section>
        <SectionHeading
          title="In-app alerts"
          detail="Choose which server-derived events appear in your notification center."
        />
        <div className="border-y border-[var(--line)]">
          {notificationCatalog.map(([type, label, detail]) => (
            <div
              key={type}
              className="grid gap-3 border-t border-[var(--line)] py-4 first:border-0 sm:grid-cols-[minmax(0,1fr)_auto_auto_auto] sm:items-center"
            >
              <div>
              ...
              <button
                type="button"
                role="switch"
                aria-checked={localInApp[type]}
                disabled={pending}
                onClick={() => saveInApp(type, !localInApp[type])}
                className={cn(
                  "focus-ring flex h-8 items-center gap-2 rounded-full px-2.5 text-xs font-semibold",
                  localInApp[type]
                    ? "bg-[var(--positive-soft)] text-[var(--positive)]"
              ...
              >
                ...
                />
                {localInApp[type] ? "In app on" : "In app off"}
              </button>
              ...
              <StatusPill
                tone={
                  type === "reservation_changed" && reservationPushEnabled
                    ? "positive"
                    : "neutral"
                }
              >
        ...
        <p className="mt-3 text-xs leading-4 text-[var(--ink-faint)]">
          Reservation-change push is available on registered browsers when the
          location owner enables staff push and the delivery worker is
          configured. Other push and staff email channels remain inactive.
        </p>
      </section>
      <section>
        <SectionHeading
          title="Browser push"
          detail="Register this device for encrypted, server-delivered reservation alerts."
          action={
            <Button
              variant="secondary"
              size="sm"
              disabled={pending || !vapidPublicKey}
              onClick={registerBrowser}
            >
              {pending ? (
        ...
        <div className="border-y border-[var(--line)]">
          {data.pushSubscriptions.map((subscription) => (
            ...
            >
              <span className="flex size-8 items-center justify-center rounded-xl bg-[var(--canvas-strong)]">
                <Smartphone className="size-3.5 text-[var(--ink-soft)]" />
              </span>
              <div className="min-w-0 flex-1">
                <p className="text-xs font-semibold">
                  {subscription.deviceLabel ?? "Stored browser"}
                </p>
                <p className="mt-1 font-mono text-xs text-[var(--ink-faint)]">
                  Endpoint {subscription.endpointHash.slice(0, 12)}… · stored{" "}
              ...
              <Button
                ...
                size="sm"
                disabled={pending}
                onClick={() => removeBrowser(subscription.endpointHash)}
              >
                Remove
              </Button>
            </div>
          ...
          {!data.pushSubscriptions.length ? (
            <p className="py-5 text-center text-xs text-[var(--ink-faint)]">
              No encrypted browser subscription is stored for your account.
            </p>
          ) : null}
        </div>
        <div className="mt-4 flex items-start gap-3 rounded-[16px] bg-[var(--positive-soft)] p-4 text-xs leading-4 text-[var(--positive)]">
          <BellRing className="mt-0.5 size-4 shrink-0" />
          <span>
            Browser endpoints are encrypted at rest. Delivery is deduplicated,
            retried up to five times, and stale subscriptions are removed when
      ...
      {message ? (
        <p
          role="status"
          aria-live="polite"
          className="rounded-xl bg-[var(--canvas)] px-4 py-3 text-xs"
        >
          {message}
        </p>
      ) : null}
...
function ExpenseCategoriesPanel({
  ...
  const [message, setMessage] = useState<string | null>(null);
  const { requestIdFor, rotateRequestId } = useStableRequestIds();
  const saveScope = editingId
    ? `expense-category.save:${editingId}`
  ...
  function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      rotateRequestId(saveScope);
      setMessage(
        editingId ? "Expense category updated." : "Expense category created.",
      );
      setEditingId(null);
      setName("");
      setAccountingCode("");
  ...
  function setActive(categoryId: string, active: boolean) {
    ...
    startTransition(async () => {
      ...
      setMessage(
        active ? "Expense category restored." : "Expense category deactivated.",
      );
      router.refresh();
    });
  }
    ...
    <div className="space-y-7">
      <section>
        <SectionHeading
          ...
          detail="Shared receipt-coding options. Deactivation preserves historical references."
        />
        <form
          onSubmit={submit}
          className="grid gap-3 border-y border-[var(--line)] py-5 sm:grid-cols-[minmax(0,1fr)_180px_auto]"
        >
          <label>
            <span className="mb-1.5 block text-xs font-semibold text-[var(--ink-faint)]">
              Category name
            </span>
            <input
              required
      ...
      <section className="border-y border-[var(--line)]">
        {data.expenseCategories.map((category) => (
          <div
            key={category.id}
            className="flex flex-wrap items-center gap-3 border-t border-[var(--line)] py-4 first:border-0"
          >
            <span
              className={cn(
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 text-xs text-[var(--ink-faint)]">
                {category.accountingCode
                  ? `Code ${category.accountingCode}`
                  : "No accounting code"}
              </p>
            </div>
            <StatusPill tone={category.active ? "positive" : "neutral"}>
            ...
            <Button
              ...
              size="sm"
              disabled={pending}
              onClick={() => setActive(category.id, !category.active)}
            >
        ...
        {!data.expenseCategories.length ? (
          <p className="py-8 text-center text-xs text-[var(--ink-faint)]">
            No expense categories exist yet. Add the first coding option above.
          </p>
        ) : null}
      </section>
      {message ? (
    ...
    <div className="space-y-9">
      <section>
        <SectionHeading
          ...
          detail="Configuration evidence, backup status, and unresolved operational errors."
        />
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="rounded-[18px] border border-[var(--line)] p-4">
            <Archive className="size-4 text-[var(--warning)]" />
            <p className="mt-4 text-xs font-semibold">Retention classes</p>
            <p className="numeric mt-1 text-lg font-semibold">
              {data.retentionPolicies.length}
            </p>
            <StatusPill
              className="mt-3"
              tone={data.retentionPolicies.length ? "positive" : "warning"}
            >
              {data.retentionPolicies.length ? "Recorded" : "Owner decision"}
            </StatusPill>
          </div>
          <div className="rounded-[18px] border border-[var(--line)] p-4">
            <DatabaseBackup className="size-4 text-[var(--ink-soft)]" />
            <p className="mt-4 text-xs font-semibold">Latest backup evidence</p>
            <p className="mt-1 text-xs text-[var(--ink-faint)]">
              {data.latestBackup
                ? `${data.latestBackup.provider} · ${dateTime(data.latestBackup.completedAt)}`
                : "No run recorded"}
            </p>
            <StatusPill
              className="mt-3"
              tone={
                data.latestBackup ? tone(data.latestBackup.status) : "warning"
              }
            >
              {data.latestBackup?.status ?? "Not active"}
            </StatusPill>
          </div>
          <div className="rounded-[18px] border border-[var(--line)] p-4">
            <MonitorCheck className="size-4 text-[var(--ink-soft)]" />
            <p className="mt-4 text-xs font-semibold">Unresolved errors</p>
            <p className="numeric mt-1 text-lg font-semibold">
              {data.unresolvedErrorCount}
            </p>
            <StatusPill
              className="mt-3"
              tone={data.unresolvedErrorCount ? "warning" : "positive"}
            >
              {data.unresolvedErrorCount ? "Review" : "Clear"}
            </StatusPill>
          </div>
        </div>
      </section>
      <RetentionPolicyConfiguration
        workspace={workspace}
        policies={data.retentionPolicies}
      ...
      <section>
        ...
        <div className="border-y border-[var(--line)]">
          {data.exportRequests.map((request) => (
            <div
              key={request.id}
              className="flex items-center gap-3 border-t border-[var(--line)] py-3.5 first:border-0"
            >
              <span className="flex size-8 items-center justify-center rounded-xl bg-[var(--canvas-strong)]">
                <DatabaseBackup className="size-3.5 text-[var(--ink-soft)]" />
              </span>
              <div className="min-w-0 flex-1">
                <p className="text-xs font-semibold capitalize">
                  {request.subjectType} export
                </p>
                <p className="mt-1 text-xs text-[var(--ink-faint)]">
      ...
      <section>
        ...
        <div className="overflow-x-auto border-y border-[var(--line)]">
          ...
          {data.auditEvents.map((event) => (
            <div
              key={event.id}
              className="grid min-w-[700px] grid-cols-[1fr_1fr_.8fr_.8fr] items-center gap-4 border-t border-[var(--line)] px-4 py-3.5"
            >
              <span className="text-xs font-semibold">
                {event.action.replaceAll("_", " ")}
              </span>
              <span className="truncate font-mono text-xs text-[var(--ink-faint)]">
          ...
          {!data.auditEvents.length ? (
            <p className="py-5 text-center text-xs text-[var(--ink-faint)]">
              No audit event is visible in this scope.
            </p>
          ) : null}
        </div>
      </section>
...
export function LiveSettingsWorkspace({
  ...
  workspace: WorkspaceContextValue;
  result: LiveReadResult<LiveSettingsModel>;
}) {
  ...
  return (
    <PageFrame width="standard">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-1 text-[13px] text-[var(--ink-faint)]">
            Organization, locations, job-role capabilities, security,
            notifications, retention, and audit
          </p>
        </div>
        <StatusPill
          tone={data.organization.configuredAt ? "positive" : "warning"}
        >
      ...
      <section className="mt-5 grid grid-cols-2 divide-x divide-y divide-[var(--line)] border-y border-[var(--line)] sm:grid-cols-4 sm:divide-y-0">
        <Metric
          label="Owners"
          value={String(data.roleCounts.owner)}
          detail={`${data.membershipCounts.active} active members`}
        />
        <Metric
          label="Locations"
          value={String(data.locations.length)}
          detail="Current access scope"
        />
        <Metric
          label="Session"
          value={workspace.identity.aal.toUpperCase()}
          detail={`${workspace.role} access`}
        />
        <Metric
          label="Retention"
          value={data.retentionPolicies.length ? "Recorded" : "Unset"}
          detail={
            data.retentionPolicies.length
              ? `${data.retentionPolicies.length} classes`
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[210px_1fr]">
          ...
          {tabs.map((item) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "focus-ring flex min-h-10 shrink-0 items-center gap-2.5 rounded-xl px-3 text-left text-[13px] font-semibold transition-colors lg:w-full",
                  active
                    ? "bg-[var(--ink)] text-[var(--paper)]"
                    : "text-[var(--ink-faint)] hover:bg-[var(--canvas-strong)] hover:text-[var(--ink)]",
                )}
              >
        ...
        <div className="min-w-0">
          {tab === "organization" ? (
            <OrganizationPanel data={data} />
          ) : tab === "locations" ? (
            <LocationsPanel data={data} />
          ) : tab === "capabilities" ? (
            <CapabilityConfiguration data={data} workspace={workspace} />
          ) : tab === "expenses" ? (
            <ExpenseCategoriesPanel data={data} workspace={workspace} />
          ) : tab === "security" ? (
            <SecurityPanel workspace={workspace} />
          ) : tab === "notifications" ? (
            <NotificationsPanel data={data} workspace={workspace} />
          ) : (
            <DataPanel data={data} workspace={workspace} />
          )}
        </div>
      </div>
    </PageFrame>
