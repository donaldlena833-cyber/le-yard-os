          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-[13px] leading-5 text-[var(--ink-faint)]">They’ll receive a one-time invitation and create their own password. Existing passwords are never visible.</p>
              </div>
...
function DemoTeamWorkspace({ workspace }: { workspace: WorkspaceContextValue }) {
  ...
  return (
    <PageFrame width="wide">
      ...
      <div className="mt-6 grid min-h-[720px] overflow-hidden rounded-[22px] border border-[var(--line)] bg-[var(--paper)] xl:grid-cols-[390px_minmax(0,1fr)]">
        <section className="border-b border-[var(--line)] xl:border-r xl:border-b-0" aria-label="Team directory">
          ...
          <div className="max-h-[690px] overflow-y-auto">
            ...
            {!filteredPeople.length ? <div className="px-5 py-14 text-center"><Search className="mx-auto size-5 text-[var(--ink-faint)]" /><p className="mt-3 text-xs font-semibold">No people match</p><p className="mt-1 text-xs text-[var(--ink-faint)]">Try a different search or filter.</p></div> : null}
          </div>
        </section>
          ...
          <div className="border-b border-[var(--line)] p-5 sm:p-7">
            <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-start">
              <div className="flex items-center gap-4">
                ...
                <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="text-xl font-medium tracking-[-0.04em]">{selected.displayName}</h3><StatusPill tone={roleTone[selectedRole]}>{roleLabel[selectedRole]}</StatusPill><StatusPill tone={selectedStatus === "active" ? "positive" : selectedStatus === "invited" ? "warning" : "danger"} dot={selectedStatus === "active"}>{selectedStatus}</StatusPill></div><p className="mt-1 text-[13px] text-[var(--ink-faint)]">{selectedJobs.map((job) => job.name).join(" · ")} · joined {selected.hiredOn}</p></div>
              </div>
            ...
            {!canManageRole ? <p className="mt-4 flex items-center gap-2 rounded-xl bg-[var(--canvas)] px-3 py-2.5 text-xs text-[var(--ink-faint)]"><LockKeyhole className="size-3.5" />Your own owner role is protected. Account ownership changes require another owner.</p> : null}
          </div>
          ...
          <div className="grid gap-8 p-5 sm:p-7 lg:grid-cols-2">
            <section>
              ...
              <div className="grid grid-cols-7 gap-1.5" aria-label="Weekly availability">
                {selected.availability.map((entry) => <div key={entry.weekday} className={cn("rounded-xl border px-1 py-2.5 text-center", entry.available ? "border-[var(--positive)]/20 bg-[var(--positive-soft)]" : "border-[var(--line)] bg-[var(--canvas)]")}><p className="text-xs font-semibold">{["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"][entry.weekday]}</p><p className={cn("mt-1 text-xs", entry.available ? "text-[var(--positive)]" : "text-[var(--ink-faint)]")}>{entry.available ? entry.startsAtLocal : "Off"}</p></div>)}
              </div>
              <div className="mt-5">
                <p className="text-xs font-semibold">Time off</p>
                {selectedTimeOff.length ? <div className="mt-2 space-y-2">{selectedTimeOff.map((request) => <div key={request.id} className="flex items-start justify-between gap-3 rounded-xl border border-[var(--line)] px-3 py-3"><div><p className="text-xs font-semibold capitalize">{request.kind}</p><p className="numeric mt-1 text-xs text-[var(--ink-faint)]">{request.startsOn}{request.endsOn !== request.startsOn ? `–${request.endsOn}` : ""}</p></div><StatusPill tone={request.status === "approved" ? "positive" : "warning"}>{request.status}</StatusPill></div>)}</div> : <p className="mt-2 text-xs text-[var(--ink-faint)]">No requests on file.</p>}
              </div>
            </section>
            ...
            <section>
              ...
              <div className="space-y-2">
                {selectedCertifications.map((certification) => <div key={certification.id} className="flex items-center gap-3 rounded-xl border border-[var(--line)] px-3 py-3"><span className={cn("flex size-8 items-center justify-center rounded-xl", certification.status === "current" ? "bg-[var(--positive-soft)] text-[var(--positive)]" : "bg-[var(--warning-soft)] text-[var(--warning)]")}><BadgeCheck className="size-4" /></span><div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold">{certification.name}</p><p className="mt-1 text-xs text-[var(--ink-faint)]">{certification.expiresOn ? `Expires ${certification.expiresOn}` : "No expiry"}</p></div><StatusPill tone={certification.status === "current" ? "positive" : "warning"}>{certification.status}</StatusPill></div>)}
                {selectedDocuments.map((document) => <div key={document.id} className="flex items-center gap-3 rounded-xl border border-[var(--line)] px-3 py-3"><FileCheck2 className="size-4 text-[var(--ink-faint)]" /><div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold">{document.title}</p><p className="mt-1 text-xs text-[var(--ink-faint)]">Private · {Math.round(document.file.byteSize / 1000)} KB</p></div>{document.acknowledgedAt ? <Check className="size-3.5 text-[var(--positive)]" aria-label="Acknowledged" /> : null}</div>)}
                {!selectedCertifications.length && !selectedDocuments.length ? <div className="rounded-xl border border-dashed border-[var(--line)] px-4 py-7 text-center text-xs text-[var(--ink-faint)]">No certifications or documents attached.</div> : null}
              </div>
            </section>

            <section>
              <SectionHeading eyebrow="Private" title="Emergency contact" detail="Visible only to authorized management" />
              {canSeeEmergency && selected.emergencyContact ? <div className="rounded-2xl bg-[var(--canvas)] p-4"><p className="text-xs font-semibold">{selected.emergencyContact.name}</p><p className="mt-1 text-xs text-[var(--ink-faint)]">{selected.emergencyContact.relationship}</p><p className="numeric mt-3 flex items-center gap-2 text-xs"><Phone className="size-3.5 text-[var(--ink-faint)]" />{selected.emergencyContact.phone}</p></div> : <div className="rounded-2xl border border-dashed border-[var(--line)] p-4 text-xs text-[var(--ink-faint)]">{selected.emergencyContact ? "You do not have permission to view this field." : "Not provided yet."}</div>}
            </section>
      ...
      <div aria-live="polite" className="mt-4 min-h-5 text-xs text-[var(--positive)]">{notice ? <span className="inline-flex items-center gap-2"><Check className="size-3.5" />{notice}</span> : null}</div>
      <div className="mt-2 flex items-start gap-3 rounded-2xl bg-[var(--accent-soft)]/40 px-4 py-3 text-xs leading-4 text-[var(--ink-soft)]"><CircleAlert className="mt-0.5 size-3.5 shrink-0 text-[var(--accent-strong)]" /><span>Only owners and admins can create, invite, suspend, or assign access. Teammates set their own passwords; no password is stored or shown here.</span></div>
      <InviteDialog
        open={inviteOpen}
      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-xs text-[var(--ink-faint)]">{workspace.organization.name}</span>
          </div>
          <h2 className="mt-3 text-2xl font-medium tracking-[-0.045em]">Secure team provisioning</h2>
          <p className="mt-1 text-[13px] text-[var(--ink-faint)]">Invite teammates into the server-verified organization and location scope.</p>
        </div>
        <Button
          variant="accent"
          ...
          {workspace.locations.map((location) => (
            <div key={location.id} className="flex items-center gap-3 rounded-2xl bg-[var(--canvas)] px-4 py-3">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-0.5 text-xs text-[var(--ink-faint)]">Available invitation scope</p>
              </div>
            </div>
          ))}
        </div>
