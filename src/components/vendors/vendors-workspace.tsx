      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-[var(--ink-faint)]">Le Yard · {workspace.persona === "chef" ? "Chef view" : "Back office"}</span>
          </div>
          <h2 className="mt-3 text-2xl font-medium tracking-[-0.045em]">Vendors & prices</h2>
          <p className="mt-1 text-[11px] text-[var(--ink-faint)]">Compare current food costs, review recent price movement, and see open orders before building the kitchen plan.</p>
        </div>
        ...
        <section>
          <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
            ...
            <label className="relative block sm:w-56">
              ...
              <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search vendors" className="h-10 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] pr-3 pl-9 text-[11px] outline-none focus:border-[var(--accent)]" />
            </label>
          </div>
          <div className="mt-4 border-y border-[var(--line)]">
            {filteredVendors.map((vendor) => (
              <button type="button" key={vendor.id} onClick={() => setSelectedVendorId(vendor.id)} className={`flex w-full items-center gap-3 border-t border-[var(--line)] px-3 py-4 text-left first:border-0 hover:bg-[var(--paper)] ${vendor.id === selectedVendor?.id ? "bg-[var(--paper)]" : ""}`}>
                ...
                <span className="min-w-0 flex-1"><span className="block truncate text-xs font-semibold">{vendor.name.replace(" — Demo", "")}</span><span className="mt-1 block text-[10px] text-[var(--ink-faint)]">{vendor.contactName} · {vendor.paymentTerms}</span></span>
