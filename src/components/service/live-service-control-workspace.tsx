...
export function LiveServiceControlWorkspace({ workspace, result }: { workspace: WorkspaceContextValue; result: LiveReadResult<LiveServiceControlModel> }) {
  ...
  return <PageFrame>
    ...
    <section className="mt-8"><SectionHeading eyebrow="Live availability" title="Running low & 86" detail="Internal status only; Toast is not changed." />
      {model.canManageAvailability ? <form onSubmit={availabilitySubmit} className="grid gap-3 rounded-2xl border border-[var(--line)] bg-[var(--paper-strong)] p-4 md:grid-cols-6"><select name="subjectType" className={field}><option value="menu_item">Menu item</option><option value="component">Component</option></select><input required name="subjectLabel" placeholder="Item or component" className={`${field} md:col-span-2`} /><select name="status" className={field}><option value="running_low">Running low</option><option value="eighty_sixed">86</option><option value="restored">Restored</option><option value="available">Available</option></select><input name="estimatedPortions" type="number" min="0" step="0.001" placeholder="Portions" className={field} /><Button type="submit" variant="accent" disabled={busy}>{busy ? <LoaderCircle className="size-4 animate-spin" /> : <Radio className="size-4" />}Update</Button><input name="reason" placeholder="Reason (optional)" className={`${field} md:col-span-3`} /><input name="notes" placeholder="Team note (optional)" className={`${field} md:col-span-3`} /></form> : null}
      <div className="mt-4 divide-y divide-[var(--line)] border-y border-[var(--line)]">{model.availability.map((item) => <div key={item.id} className="flex items-center gap-3 py-3"><span className="min-w-0 flex-1"><span className="block text-xs font-semibold">{item.subjectLabel}</span><span className="mt-1 block text-[10px] text-[var(--ink-faint)]">{item.reason || item.notes || "No note"}</span></span>{item.estimatedPortions != null ? <span className="numeric text-xs">{item.estimatedPortions} portions</span> : null}<StatusPill tone={item.status === "eighty_sixed" ? "danger" : item.status === "running_low" ? "warning" : "positive"}>{item.status === "eighty_sixed" ? "86" : sentence(item.status)}</StatusPill></div>)}{!model.availability.length ? <p className="py-8 text-center text-xs text-[var(--ink-faint)]">No availability events yet.</p> : null}</div>
    </section>
    <div className="mt-9 grid gap-9 xl:grid-cols-2">
      <section><SectionHeading eyebrow="Handoff" title="Manager Log" detail="Unresolved entries carry into the next service." />
        {model.canManageLog ? <form onSubmit={managerLogSubmit} className="grid gap-3 rounded-2xl border border-[var(--line)] p-4 sm:grid-cols-2"><input required name="title" placeholder="Handoff title" className={`${field} sm:col-span-2`} /><textarea required name="narrative" placeholder="What happened and what needs follow-up?" className={`${area} sm:col-span-2`} /><select name="servicePeriod" className={field}><option value="dinner">Dinner</option><option value="lunch">Lunch</option><option value="all_day">All day</option><option value="other">Other</option></select><select name="category" className={field}><option value="foh">FOH</option><option value="boh">BOH</option><option value="guest">Guest</option><option value="equipment">Equipment</option><option value="inventory">Inventory</option><option value="maintenance">Maintenance</option><option value="other">Other</option></select><select name="severity" className={field}><option value="awareness">Awareness</option><option value="action_required">Action required</option><option value="critical">Critical</option><option value="informational">Informational</option></select><select name="status" className={field}><option value="needs_follow_up">Needs follow-up</option><option value="informational">Informational</option><option value="in_progress">In progress</option></select><Button type="submit" variant="secondary" disabled={busy} className="sm:col-span-2"><Plus className="size-4" />Add handoff</Button></form> : null}
        <div className="mt-4 space-y-3">{model.managerLog.map((entry) => <article key={entry.id} className="rounded-2xl border border-[var(--line)] p-4"><div className="flex items-start gap-3"><ClipboardPenLine className="mt-0.5 size-4 text-[var(--accent-strong)]" /><div className="min-w-0 flex-1"><p className="text-xs font-semibold">{entry.title}</p><p className="mt-2 text-[11px] leading-5 text-[var(--ink-faint)]">{entry.narrative}</p></div><StatusPill tone={entry.severity === "critical" ? "danger" : entry.severity === "action_required" ? "warning" : "neutral"}>{sentence(entry.severity)}</StatusPill></div></article>)}{!model.managerLog.length ? <p className="py-6 text-center text-xs text-[var(--ink-faint)]">No unresolved handoffs.</p> : null}</div>
      </section>
      <section><SectionHeading eyebrow="Before service" title="Pre-shift" detail="Publish facts; employees acknowledge what they read." />
        {model.canManagePreshift ? <form onSubmit={preshiftSubmit} className="grid gap-3 rounded-2xl border border-[var(--line)] p-4 sm:grid-cols-2"><select name="servicePeriod" className={field}><option value="dinner">Dinner</option><option value="lunch">Lunch</option><option value="all_day">All day</option></select><input name="bookedCovers" type="number" min="0" placeholder="Booked covers (if known)" className={field} /><textarea name="vipNotes" placeholder="VIP notes" className={area} /><textarea name="allergyNotes" placeholder="Allergies" className={area} /><textarea name="largePartyNotes" placeholder="Large parties" className={area} /><textarea name="specials" placeholder="Specials and 86 context" className={area} /><textarea name="staffingNotes" placeholder="Staffing notes" className={area} /><textarea name="serviceGoal" placeholder="Service goal" className={area} /><input name="trainingPoint" placeholder="Training point" className={`${field} sm:col-span-2`} /><textarea name="managerNotes" placeholder="Manager notes" className={`${area} sm:col-span-2`} /><select name="status" className={field}><option value="draft">Save draft</option><option value="published">Publish now</option></select><Button type="submit" variant="accent" disabled={busy}><BookOpenCheck className="size-4" />Save pre-shift</Button></form> : null}
        <div className="mt-4 space-y-3">{model.preshifts.map((preshift) => <article key={preshift.id} className="rounded-2xl border border-[var(--line)] p-4"><div className="flex items-center gap-3"><span className="min-w-0 flex-1"><span className="block text-xs font-semibold">{sentence(preshift.servicePeriod)} · {preshift.businessDate}</span><span className="mt-1 block text-[10px] text-[var(--ink-faint)]">{preshift.bookedCovers == null ? "No reservation total connected" : `${preshift.bookedCovers} booked covers`} · {preshift.acknowledgementCount} acknowledged</span></span><StatusPill tone={preshift.status === "published" ? "positive" : "neutral"}>{sentence(preshift.status)}</StatusPill></div>{preshift.status === "published" && !preshift.acknowledgedByCurrentEmployee ? <Button size="sm" variant="secondary" disabled={busy} className="mt-4" onClick={() => void perform(acknowledgePreshiftAction({ requestId: crypto.randomUUID(), preshiftId: preshift.id, comment: null }), "Pre-shift acknowledged.")}><Check className="size-4" />Acknowledge</Button> : null}</article>)}{!model.preshifts.length ? <div className="flex items-center gap-2 rounded-xl bg-[var(--warning-soft)] p-4 text-xs text-[var(--warning)]"><AlertTriangle className="size-4" />No pre-shift has been created for this service.</div> : null}</div>
      </section>
    </div>
  </PageFrame>;
}
