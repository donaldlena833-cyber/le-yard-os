  ...
  return (
    <label className="grid grid-cols-[minmax(0,1fr)_132px] items-center gap-4 border-t border-[var(--line)] py-3 first:border-t-0">
      ...
      <span className={cn("flex h-10 items-center rounded-xl border bg-[var(--paper-strong)] px-3", valid ? "border-[var(--line)]" : "border-[var(--danger)]")}>
        <span className="mr-1 text-[9px] font-semibold text-[var(--ink-faint)]">{currencyCode}</span>
        ...
        {run.allocations.map((allocation) => {
          ...
          return (
            <div key={allocation.id} className="grid grid-cols-[1fr_auto] gap-3 border-t border-[var(--line)] px-4 py-4 first:border-t-0 sm:grid-cols-[1fr_120px_120px]">
              <div><p className="text-xs font-semibold">{allocation.employeeName}</p><p className="mt-1 text-[9px] leading-4 text-[var(--ink-faint)]">{String(explanation.method ?? run.method ?? "policy")} · {String(explanation.worked_minutes ?? "—")} minutes · remainder rank {allocation.remainderRank ?? "—"}</p></div>
              <span className="numeric hidden text-right text-[10px] text-[var(--ink-faint)] sm:block">{formatCurrency(allocation.baseAmountCents, currencyCode)} base</span>
              <span className="numeric text-right text-xs font-semibold">{formatCurrency(allocation.finalAmountCents, currencyCode)}</span>
            </div>
          );
        })}
        {!run.allocations.length ? <p className="px-5 py-12 text-center text-[10px] text-[var(--ink-faint)]">Calculate this run to create deterministic allocations.</p> : null}
      </div>
    </div>
  );
}
...
export function LiveCloseoutWorkspace({
  ...
  return (
    ...
    <PageFrame width="full" className="max-w-[1700px]">
      <header className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><div className="flex items-center gap-2"><StatusPill tone="positive" dot>Connected</StatusPill><span className="text-[10px] text-[var(--ink-faint)]">Human approval · immutable locks</span></div><h2 className="mt-3 text-2xl font-medium tracking-[-0.045em]">Closeout & tips</h2><p className="mt-1 text-[11px] text-[var(--ink-faint)]">Sales, cash, source-separated tips, and payroll support for {workspace.activeLocation.name}.</p></div><Button variant="accent" onClick={newCloseout}><Plus className="size-4" />New closeout</Button></header>

      <div className="mt-7 grid gap-10 xl:grid-cols-[260px_minmax(0,1fr)_minmax(340px,.8fr)]">
        <aside><SectionHeading eyebrow="History" title="Recent closeouts" detail={`${model.closeouts.length} visible records`} /><div className="space-y-2">{model.closeouts.map((closeout) => <button key={closeout.id} onClick={() => selectCloseout(closeout)} className={cn("focus-ring w-full rounded-[16px] border p-4 text-left", selectedId === closeout.id ? "border-[var(--accent)] bg-[var(--accent-soft)]/35" : "border-[var(--line)] bg-[var(--paper-strong)]")}><div className="flex items-center justify-between gap-2"><span className="text-xs font-semibold">{closeout.shiftLabel}</span><StatusPill tone={closeoutTone[closeout.status] ?? "neutral"}>{closeout.status}</StatusPill></div><p className="numeric mt-2 text-[10px] text-[var(--ink-faint)]">{closeout.businessDate} · {formatMoney(closeout.netSalesCents)}</p></button>)}{!model.closeouts.length ? <p className="rounded-[16px] border border-[var(--line)] p-5 text-[10px] text-[var(--ink-faint)]">No connected closeouts yet.</p> : null}</div></aside>
        ...
        <main>
          ...
          <form onSubmit={(event) => void submit(event)}>
            ...
            {selected && ["pending", "in_review"].includes(selected.status) ? <div className="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-[var(--line)] pt-5"><p className="text-[10px] text-[var(--ink-faint)]">{selected.submittedByUserId === workspace.identity.userId ? "A different manager must decide this submission." : "Approval permanently locks the closeout evidence."}</p>{selected.submittedByUserId !== workspace.identity.userId ? <div className="flex gap-2"><Button variant="danger" disabled={busy} onClick={() => void perform(approveCloseoutAction({ closeoutId: selected.id, approved: false, note: null }), "Closeout rejected and locked with its original evidence.")}><X className="size-4" />Reject</Button><Button variant="accent" disabled={busy} onClick={() => void perform(approveCloseoutAction({ closeoutId: selected.id, approved: true, note: null }), "Closeout approved and locked.")}><Check className="size-4" />Approve</Button></div> : null}</div> : null}
          </form>
        </main>
