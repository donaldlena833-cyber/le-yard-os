...
function ChannelRow({
  ...
  return (
    <button type="button" onClick={onSelect} aria-pressed={selected} className={cn("focus-ring group flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left transition-colors hover:bg-[var(--paper)]", selected && "bg-[var(--paper-strong)] shadow-sm")}>
      ...
      <span className="min-w-0 flex-1"><span className={cn("block truncate text-[13px]", unread ? "font-bold text-[var(--ink)]" : "font-semibold text-[var(--ink-soft)]")}>{channel.name}</span><span className="mt-0.5 block truncate text-xs text-[var(--ink-faint)]">{channelDescription(channel)}</span></span>
      {unread ? <span className="numeric flex min-w-5 items-center justify-center rounded-full bg-[var(--accent)] px-1.5 py-1 text-xs font-bold text-white" aria-label={`${unread} unread`}>{unread}</span> : <ChevronRight className="size-3.5 text-[var(--ink-faint)] opacity-0 transition-opacity group-hover:opacity-100" />}
    </button>
  );
}
  ...
  return (
    <motion.article layout initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className={cn("group flex items-start gap-3", mine && "flex-row-reverse")}>
      ...
      <div className={cn("min-w-0 max-w-[min(78%,620px)]", mine && "items-end text-right")}>
        <div className={cn("mb-1 flex items-center gap-2", mine && "justify-end")}><p className="text-xs font-semibold">{author?.displayName ?? "Team member"}</p><time className="numeric text-xs text-[var(--ink-faint)]" dateTime={message.createdAt}>{formatMessageTime(message.createdAt)}</time>{message.editedAt ? <span className="text-xs text-[var(--ink-faint)]">edited</span> : null}</div>
        <div className={cn("rounded-[18px] px-3.5 py-2.5 text-left text-[13px] leading-5", mine ? "rounded-tr-md bg-[var(--ink)] text-[var(--paper)] dark:bg-[var(--accent)] dark:text-[#171a17]" : "rounded-tl-md border border-[var(--line)] bg-[var(--paper-strong)] text-[var(--ink)]")}>
          <p>{message.body}</p>
          {message.attachmentIds.map((attachmentId) => <button key={attachmentId} type="button" className={cn("mt-2 flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left", mine ? "bg-white/10" : "bg-[var(--canvas)]")}><FileText className="size-3.5 shrink-0" /><span className="min-w-0 flex-1 truncate text-xs font-semibold">{attachmentId.includes("produce") ? "demo-produce-invoice.pdf" : "private-attachment"}</span><span className="text-xs opacity-60">Private</span></button>)}
        </div>
        <div className={cn("mt-1.5 flex flex-wrap items-center gap-1", mine && "justify-end")}>
          {message.reactions.map((reaction) => <button key={reaction.emoji} type="button" onClick={() => onToggleReaction(reaction.emoji)} aria-label={`${reaction.emoji} reaction from ${reaction.personIds.length} people`} className={cn("focus-ring inline-flex min-h-6 items-center gap-1 rounded-full border px-2 text-xs", reaction.personIds.includes(currentUserId) ? "border-[var(--accent)] bg-[var(--accent-soft)]" : "border-[var(--line)] bg-[var(--paper)]")}><span>{reaction.emoji}</span><span className="numeric text-[var(--ink-faint)]">{reaction.personIds.length}</span></button>)}
        ...
        {mine ? <p className="mt-1.5 flex items-center justify-end gap-1 text-xs text-[var(--ink-faint)]"><Check className="size-2.5" />{message.readBy.length ? `Read by ${message.readBy.length}` : "Sent"}</p> : null}
      </div>
    </motion.article>
  );
}
  ...
  return (
    <PageFrame width="full" className="max-w-[1680px] pb-3 lg:pb-8">
      <div className="mb-5 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div><div className="flex items-center gap-2"><StatusPill tone={totalUnread ? "accent" : "positive"} dot>{totalUnread ? `${totalUnread} unread` : "Caught up"}</StatusPill><span className="text-xs text-[var(--ink-faint)]">Realtime team workspace</span></div><h2 className="mt-3 text-2xl font-medium tracking-[-0.045em]">Stay close to service</h2><p className="mt-1 text-[13px] text-[var(--ink-faint)]">One place for all-staff updates, location conversations, and private management work.</p></div>
      ...
      <div className="grid min-h-[calc(100svh-190px)] overflow-hidden rounded-[24px] border border-[var(--line)] bg-[var(--paper)] shadow-[0_12px_42px_rgba(25,28,24,.04)] lg:grid-cols-[270px_minmax(0,1fr)] xl:grid-cols-[270px_minmax(0,1fr)_290px]">
        <nav aria-label="Message channels" className={cn("border-[var(--line)] bg-[var(--canvas)] p-3 lg:block lg:border-r", mobileChatOpen ? "hidden" : "block")}>
          ...
          <div className="mt-8 rounded-2xl border border-[var(--line)] bg-[var(--paper)] p-3"><div className="flex -space-x-1.5">{realPlaygroundPeople.map((person, index) => <Avatar key={person.id} name={person.displayName} size="sm" index={index} />)}</div><p className="mt-3 text-xs font-semibold">{realPlaygroundPeople.length} Le Yard users</p><p className="mt-1 text-xs leading-4 text-[var(--ink-faint)]">Messages sync across signed-in Le Yard sessions in this browser.</p></div>
        </nav>
        ...
        <main className={cn("min-w-0 flex-col bg-[var(--paper-strong)] lg:flex", mobileChatOpen ? "flex" : "hidden")} aria-label={`${selectedChannel.name} conversation`}>
          <header className="flex min-h-[72px] items-center gap-3 border-b border-[var(--line)] px-3 sm:px-5"><Button variant="quiet" size="icon" className="lg:hidden" aria-label="Back to channels" onClick={() => setMobileChatOpen(false)}><ArrowLeft className="size-4" /></Button><span className={cn("flex size-9 items-center justify-center rounded-xl", selectedChannel.kind === "management" ? "bg-[var(--danger-soft)] text-[var(--danger)]" : "bg-[var(--accent-soft)] text-[var(--accent-strong)]")}>{selectedChannel.kind === "management" ? <LockKeyhole className="size-4" /> : selectedChannel.kind === "all_staff" ? <UsersRound className="size-4" /> : <Hash className="size-4" />}</span><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><h3 className="truncate text-sm font-semibold">{selectedChannel.name}</h3>{selectedChannel.kind === "management" ? <StatusPill tone="danger">Private</StatusPill> : null}</div><p className="mt-0.5 truncate text-xs text-[var(--ink-faint)]">{channelDescription(selectedChannel)}</p></div><Button variant="quiet" size="icon" aria-label="Channel information" onClick={() => setMobileInfoOpen((current) => !current)}><Info className="size-4" /></Button><Button variant="quiet" size="icon" aria-label="More channel actions"><MoreHorizontal className="size-4" /></Button></header>

          <div className="flex-1 overflow-y-auto px-3 py-5 sm:px-6 sm:py-6">
            {selectedChannel.kind === "all_staff" ? <section className="mb-6 rounded-[18px] border border-[var(--line)] bg-[var(--canvas)] p-4" aria-label="Pinned announcement"><div className="flex items-start gap-3"><span className="flex size-8 shrink-0 items-center justify-center rounded-xl bg-[var(--paper)] text-[var(--ink-faint)]"><Megaphone className="size-3.5" /></span><div className="min-w-0 flex-1"><p className="text-xs font-semibold tracking-[0.08em] uppercase">Pinned announcement · synthetic</p><p className="mt-1 text-xs font-semibold text-[var(--ink)]">{demoWorkspace.announcements[0]?.title}</p><p className="mt-1 text-xs leading-4 text-[var(--ink-soft)]">{demoWorkspace.announcements[0]?.body}</p></div></div></section> : null}
            <div className="mb-6 flex items-center gap-3"><span className="h-px flex-1 bg-[var(--line)]" /><span className="text-xs font-semibold text-[var(--ink-faint)]">Today</span><span className="h-px flex-1 bg-[var(--line)]" /></div>
            <div className="space-y-5">
          ...
          <footer className="border-t border-[var(--line)] bg-[var(--paper-strong)] p-3 sm:p-4">
            ...
            <p className="mt-2 px-2 text-xs text-[var(--ink-faint)]">Attachments stay private and use signed access links in production.</p>
          </footer>
        </main>
        ...
        <aside className={cn("border-l border-[var(--line)] bg-[var(--canvas)] p-5 xl:block", mobileInfoOpen ? "fixed inset-0 z-40 block overflow-y-auto" : "hidden")} aria-label="Channel information">
          ...
          <section className="mt-7 border-t border-[var(--line)] pt-5"><div className="flex items-center justify-between"><p className="text-xs font-semibold">Members</p><span className="numeric text-xs text-[var(--ink-faint)]">{channelMembers.length}</span></div><div className="mt-3 space-y-3">{channelMembers.slice(0, 6).map((person, index) => <div key={person.id} className="flex items-center gap-2.5"><Avatar name={person.displayName} size="sm" index={index} /><div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold">{person.displayName}</p><p className="mt-0.5 truncate text-xs text-[var(--ink-faint)]">{person.primaryRole}</p></div>{person.id === currentUserId ? <span className="text-xs text-[var(--ink-faint)]">You</span> : null}</div>)}</div>{channelMembers.length > 6 ? <button type="button" className="mt-3 text-xs font-semibold text-[var(--accent-strong)]">View all {channelMembers.length}</button> : null}</section>
          <section className="mt-7 border-t border-[var(--line)] pt-5"><p className="text-xs font-semibold">Shared files</p><p className="mt-3 rounded-xl bg-[var(--paper)] px-3 py-3 text-xs text-[var(--ink-faint)]">Files shared in this channel will appear here.</p></section>
          {selectedChannel.kind === "management" ? <div className="mt-7 flex items-start gap-2 rounded-xl bg-[var(--danger-soft)] px-3 py-3 text-xs leading-4 text-[var(--danger)]"><LockKeyhole className="mt-0.5 size-3.5 shrink-0" /><span>Membership is limited to owners and managers. Access is checked on every message.</span></div> : null}
        </aside>
      </div>
    </PageFrame>
  );
