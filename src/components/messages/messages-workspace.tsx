...
const realPlaygroundPeople = demoWorkspace.people.filter(
  (person) =>
...
function ChannelRow({
  ...
  const Icon =
    channel.kind === "management"
      ? LockKeyhole
      : channel.kind === "all_staff"
        ? UsersRound
        : Hash;
  return (
    <button
      ...
      className={cn(
        "focus-ring group flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left transition-colors hover:bg-[var(--paper)]",
        selected && "bg-[var(--paper-strong)] shadow-sm",
      )}
    >
      <span
        className={cn(
      ...
      {unread ? (
        <span
          className="numeric flex min-w-5 items-center justify-center rounded-full bg-[var(--accent)] px-1.5 py-1 text-xs font-bold text-white"
          aria-label={`${unread} unread`}
        >
          {unread}
        </span>
      ) : (
        <ChevronRight className="size-3.5 text-[var(--ink-faint)] opacity-0 transition-opacity group-hover:opacity-100" />
      )}
    </button>
  );
}
...
function MessageBubble({
  ...
  const author = realPlaygroundPeople.find(
    (person) => person.id === message.authorId,
  );
  const authorIndex = Math.max(
    0,
    realPlaygroundPeople.findIndex((person) => person.id === message.authorId),
  );
  ...
  return (
    <motion.article
      ...
      className={cn("group flex items-start gap-3", mine && "flex-row-reverse")}
    >
      <Avatar
        name={author?.displayName ?? "Team member"}
        size="sm"
        index={authorIndex}
      />
      <div
        className={cn(
          "min-w-0 max-w-[min(78%,620px)]",
          mine && "items-end text-right",
        )}
      >
        <div
          className={cn("mb-1 flex items-center gap-2", mine && "justify-end")}
        >
          <p className="text-xs font-semibold">
            {author?.displayName ?? "Team member"}
          </p>
          <time
            className="numeric text-xs text-[var(--ink-faint)]"
            dateTime={message.createdAt}
          >
            {formatMessageTime(message.createdAt, timeZone)}
          </time>
          {message.editedAt ? (
            <span className="text-xs text-[var(--ink-faint)]">edited</span>
          ) : null}
        </div>
        <div
          className={cn(
        ...
        >
          ...
          {message.attachmentIds.map((attachmentId) => (
            <button
              ...
              type="button"
              className={cn(
                "mt-2 flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left",
                mine ? "bg-white/10" : "bg-[var(--canvas)]",
              )}
            >
              <FileText className="size-3.5 shrink-0" />
              <span className="min-w-0 flex-1 truncate text-xs font-semibold">
                {attachmentId.includes("produce")
                  ? "demo-produce-invoice.pdf"
                  : "private-attachment"}
              </span>
        ...
        >
          {message.reactions.map((reaction) => (
            <button
              ...
              aria-label={`${reaction.emoji} reaction from ${reaction.personIds.length} people`}
              className={cn(
                "focus-ring inline-flex min-h-6 items-center gap-1 rounded-full border px-2 text-xs",
                reaction.personIds.includes(currentUserId)
                  ? "border-[var(--accent)] bg-[var(--accent-soft)]"
                  : "border-[var(--line)] bg-[var(--paper)]",
              )}
            >
              <span>{reaction.emoji}</span>
              <span className="numeric text-[var(--ink-faint)]">
                {reaction.personIds.length}
              </span>
            </button>
          ))}
          <div className="relative opacity-0 transition-opacity group-focus-within:opacity-100 group-hover:opacity-100">
            <details>
              <summary
                className="focus-ring flex size-6 list-none items-center justify-center rounded-full text-[var(--ink-faint)] hover:bg-[var(--canvas-strong)]"
              ...
              <div className="absolute bottom-7 left-0 z-10 flex rounded-xl border border-[var(--line)] bg-[var(--paper-strong)] p-1 shadow-lg">
                {commonReactions.map((emoji) => (
                  <button
                    key={emoji}
                    type="button"
                    className="flex size-8 items-center justify-center rounded-lg hover:bg-[var(--canvas)]"
        ...
        {mine ? (
          <p className="mt-1.5 flex items-center justify-end gap-1 text-xs text-[var(--ink-faint)]">
...
export function MessagesWorkspace() {
        ...
        .filter(
          (channel) =>
            ...
            workspace.locations.some(
              (location) => location.id === channel.locationId,
            ),
        )
        .map((channel) =>
          channel.locationId
  ...
  const channelMembers = useMemo(() => {
    if (selectedChannel.kind === "management")
      return realPlaygroundPeople.filter((person) =>
        ["owner", "manager", "admin"].includes(person.primaryRole),
      );
    if (selectedChannel.participantIds.length)
      return realPlaygroundPeople.filter((person) =>
        selectedChannel.participantIds.includes(person.id),
      );
    if (selectedChannel.locationId)
      return realPlaygroundPeople.filter((person) =>
        person.locationIds.includes(selectedChannel.locationId!),
      );
      ...
      messages.map((message) => {
        ...
        if (!existing)
          return {
            ...
            reactions: [
              ...message.reactions,
              { emoji, personIds: [currentUserId] },
            ],
          };
        const personIds = existing.personIds.includes(currentUserId)
          ? existing.personIds.filter((id) => id !== currentUserId)
          : [...existing.personIds, currentUserId];
        return {
          ...message,
          reactions: personIds.length
            ? message.reactions.map((reaction) =>
    ...
    <PageFrame width="full" className="max-w-[1680px] pb-3 lg:pb-8">
      <div className="mb-5 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          <div className="flex items-center gap-2">
            <StatusPill tone={totalUnread ? "accent" : "positive"} dot>
              {totalUnread ? `${totalUnread} unread` : "Caught up"}
            </StatusPill>
          ...
          <h2 className="mt-3 text-2xl font-medium tracking-[-0.045em]">
            Stay close to service
          </h2>
          <p className="mt-1 text-[13px] text-[var(--ink-faint)]">
            One place for all-staff updates, location conversations, and private
            management work.
          </p>
        </div>
        <Button
      ...
      <div className="grid min-h-[calc(100svh-190px)] overflow-hidden rounded-[24px] border border-[var(--line)] bg-[var(--paper)] shadow-[0_12px_42px_rgba(25,28,24,.04)] lg:grid-cols-[270px_minmax(0,1fr)] xl:grid-cols-[270px_minmax(0,1fr)_290px]">
        <nav
          ...
          className={cn(
            "border-[var(--line)] bg-[var(--canvas)] p-3 lg:block lg:border-r",
            mobileChatOpen ? "hidden" : "block",
          )}
        >
          <div className="px-2 pt-2 pb-4">
            <div className="flex items-center gap-2">
              <span className="flex size-8 items-center justify-center rounded-xl bg-[var(--graphite)] text-white">
                <MessageCircleMore className="size-3.5" />
              </span>
              <div>
                <p className="text-[13px] font-semibold">Le Yard</p>
                <p className="mt-0.5 text-xs text-[var(--ink-faint)]">
                  Team chat · main dining room
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-5">
            <section>
              <p className="px-3 text-xs font-semibold tracking-[0.12em] text-[var(--ink-faint)] uppercase">
                Team
              </p>
              <div className="mt-2 space-y-1">
                {visibleChannels
                  .filter((channel) =>
                    ["all_staff", "management"].includes(channel.kind),
                  )
                  .map((channel) => (
                    <ChannelRow
                      key={channel.id}
            ...
            <section>
              ...
              <div className="mt-2 space-y-1">
                {visibleChannels
                  .filter((channel) => channel.kind === "location")
                  .map((channel) => (
                    <ChannelRow
                      key={channel.id}
                      channel={channel}
          ...
          <div className="mt-8 rounded-2xl border border-[var(--line)] bg-[var(--paper)] p-3">
            ...
            <p className="mt-3 text-xs font-semibold">
              {realPlaygroundPeople.length} Le Yard users
            </p>
            <p className="mt-1 text-xs leading-4 text-[var(--ink-faint)]">
              Messages sync across signed-in Le Yard sessions in this browser.
            </p>
          </div>
        </nav>
        ...
        <main
          className={cn(
            "min-w-0 flex-col bg-[var(--paper-strong)] lg:flex",
            mobileChatOpen ? "flex" : "hidden",
          )}
          aria-label={`${selectedChannel.name} conversation`}
        >
          <header className="flex min-h-[72px] items-center gap-3 border-b border-[var(--line)] px-3 sm:px-5">
            <Button
              variant="quiet"
              size="icon"
              className="lg:hidden"
            ...
            <span
              className={cn(
                ...
                selectedChannel.kind === "management"
            ...
            >
              ...
              ) : selectedChannel.kind === "all_staff" ? (
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 truncate text-xs text-[var(--ink-faint)]">
                {channelDescription(selectedChannel)}
              </p>
            </div>
            <Button
              variant="quiet"
          ...
          <div className="flex-1 overflow-y-auto px-3 py-5 sm:px-6 sm:py-6">
            {selectedChannel.kind === "all_staff" ? (
              <section
                className="mb-6 rounded-[18px] border border-[var(--line)] bg-[var(--canvas)] p-4"
                aria-label="Pinned announcement"
              >
                <div className="flex items-start gap-3">
                  <span className="flex size-8 shrink-0 items-center justify-center rounded-xl bg-[var(--paper)] text-[var(--ink-faint)]">
                    <Megaphone className="size-3.5" />
                  </span>
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-semibold tracking-[0.08em] uppercase">
                      Pinned announcement · synthetic
                    </p>
                    <p className="mt-1 text-xs font-semibold text-[var(--ink)]">
                      {demoWorkspace.announcements[0]?.title}
                    </p>
                    <p className="mt-1 text-xs leading-4 text-[var(--ink-soft)]">
                      {demoWorkspace.announcements[0]?.body}
                    </p>
                  </div>
                </div>
              </section>
            ...
            <div className="space-y-5">
              ...
              {!channelMessages.length ? (
                <div className="flex min-h-64 flex-col items-center justify-center text-center">
                  <span className="flex size-12 items-center justify-center rounded-2xl bg-[var(--canvas)] text-[var(--ink-faint)]">
                    <MessageCircleMore className="size-5" />
                  </span>
                  <p className="mt-4 text-xs font-semibold">
                    Start the conversation
                  </p>
                  <p className="mt-1 max-w-xs text-xs leading-4 text-[var(--ink-faint)]">
                    Messages here are visible only to people with access to{" "}
                    {selectedChannel.name}.
                  </p>
                </div>
              ) : null}
          ...
          <footer className="border-t border-[var(--line)] bg-[var(--paper-strong)] p-3 sm:p-4">
            {attachment ? (
              <div className="mb-2 flex items-center gap-2 rounded-xl bg-[var(--canvas)] px-3 py-2 text-xs">
            ...
            >
              ...
              <label className="min-w-0 flex-1">
                <span className="sr-only">Message {selectedChannel.name}</span>
                <textarea
                  name="message"
                  value={draft}
                  ...
                  className="max-h-28 min-h-10 w-full resize-none bg-transparent px-1 py-2.5 text-xs outline-none placeholder:text-[var(--ink-faint)]"
                />
              </label>
              <Button
                type="submit"
            ...
            <p className="mt-2 px-2 text-xs text-[var(--ink-faint)]">
              Attachments stay private and use signed access links in
              production.
            </p>
          </footer>
        </main>
        ...
        <aside
          className={cn(
            "border-l border-[var(--line)] bg-[var(--canvas)] p-5 xl:block",
            mobileInfoOpen
        ...
        >
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold">Channel details</p>
            <Button
              variant="quiet"
              size="icon"
              className="xl:hidden"
          ...
          <div className="mt-6 grid grid-cols-2 gap-2">
            ...
            >
              ...
              {notificationsMuted ? "Muted" : "Notify"}
            </Button>
          ...
          <section className="mt-7 border-t border-[var(--line)] pt-5">
            ...
            <div className="mt-3 space-y-3">
              {channelMembers.slice(0, 6).map((person, index) => (
                <div key={person.id} className="flex items-center gap-2.5">
                  <Avatar name={person.displayName} size="sm" index={index} />
                  <div className="min-w-0 flex-1">
                  ...
                  {person.id === currentUserId ? (
                    <span className="text-xs text-[var(--ink-faint)]">You</span>
                  ) : null}
                </div>
              ))}
            </div>
            {channelMembers.length > 6 ? (
              <button
                type="button"
                className="mt-3 text-xs font-semibold text-[var(--accent-strong)]"
              >
                View all {channelMembers.length}
              </button>
            ) : null}
          </section>
          <section className="mt-7 border-t border-[var(--line)] pt-5">
            <p className="text-xs font-semibold">Shared files</p>
            <p className="mt-3 rounded-xl bg-[var(--paper)] px-3 py-3 text-xs text-[var(--ink-faint)]">
              Files shared in this channel will appear here.
            </p>
          </section>
          {selectedChannel.kind === "management" ? (
            <div className="mt-7 flex items-start gap-2 rounded-xl bg-[var(--danger-soft)] px-3 py-3 text-xs leading-4 text-[var(--danger)]">
              <LockKeyhole className="mt-0.5 size-3.5 shrink-0" />
              <span>
                Membership is limited to owners and managers. Access is checked
                on every message.
              </span>
            </div>
          ) : null}
