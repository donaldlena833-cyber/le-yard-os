...
import {
  acknowledgeShiftAction,
  publishScheduleAction,
} from "@/app/actions/workflows/schedule";
import {
...
function ScheduleReadError({ message }: { message: string }) {
  return (
    <PageFrame>
      <ReadState
        className="mx-auto mt-[8svh] max-w-xl"
        state="unavailable"
        title="Schedule unavailable"
        description={message}
        detail="No shift, labor, or acknowledgement values were estimated."
...
function ShiftCard({
  ...
  const state = published ? (shift.isOpen ? "open" : shift.status) : "draft";
  const context: ActionResolutionContext = {
    ...actionContext,
    satisfiedPrerequisites: [
      ...actionContext.satisfiedPrerequisites,
      "selected_shift",
      ...(mine ? (["shift_assigned_to_actor"] as const) : []),
      ...(shift.isOpen && selfEmployeeId ? (["shift_claimable"] as const) : []),
    ],
  };
  const handlers = {
    ...(editable ? { "schedule_shift.edit": onEdit } : {}),
    ...(published &&
    mine &&
    !shift.acknowledged &&
    ["scheduled", "claimed"].includes(shift.status)
      ? { "schedule_shift.acknowledge": () => onAction("ack") }
      : {}),
    ...(published &&
    mine &&
    !shift.isOpen &&
    ["scheduled", "claimed"].includes(shift.status)
      ? { "schedule_shift.request_swap": () => onAction("swap") }
      : {}),
    ...(published && shift.isOpen && selfEmployeeId
      ? { "schedule_shift.claim": () => onAction("claim") }
      : {}),
    ...(published &&
    !shift.isOpen &&
    canManage &&
    ["scheduled", "claimed", "cancelled"].includes(shift.status)
      ? { "schedule_shift.reopen": () => onAction("reopen") }
      : {}),
  };
  return (
    <article
    ...
    >
      <div className="flex items-start gap-2">
        {draggable ? (
          <button
            type="button"
            aria-label={`Move ${shift.employeeName} shift`}
            className="focus-ring -m-1 flex size-11 shrink-0 cursor-grab items-center justify-center rounded-lg text-[var(--ink-faint)] active:cursor-grabbing"
            {...listeners}
            {...attributes}
          >
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="numeric mt-2 text-xs font-semibold">
      ...
      <ObjectActionBar
        ...
        icons={{
          ...
          "schedule_shift.acknowledge": <Check className="size-3.5" />,
          "schedule_shift.claim": <UserRoundCheck className="size-3.5" />,
          "schedule_shift.request_swap": <ArrowRight className="size-3.5" />,
          "schedule_shift.reopen": <CircleDot className="size-3.5" />,
        }}
        variants={{
          "schedule_shift.edit": "quiet",
          "schedule_shift.acknowledge": "secondary",
          "schedule_shift.claim": "accent",
          "schedule_shift.request_swap": "quiet",
          "schedule_shift.reopen": "quiet",
        }}
        ariaLabels={{
          "schedule_shift.edit": `Edit ${shift.employeeName} shift`,
          "schedule_shift.acknowledge": `Acknowledge ${shift.employeeName} shift`,
          "schedule_shift.claim": shift.isOpen
            ? "Claim open shift"
            : `Claim ${shift.employeeName} shift`,
          "schedule_shift.request_swap": `Request swap for ${shift.employeeName} shift`,
          "schedule_shift.reopen": `Reopen ${shift.employeeName} shift for coverage`,
        }}
        busy={busy}
        label={
          shift.isOpen
            ? "Open shift actions"
            : `${shift.employeeName} shift actions`
        }
...
function DayColumn({
  ...
  return (
    <section
      ref={setNodeRef}
      className={cn(
        "min-h-56 rounded-[18px] border border-[var(--line)] bg-[var(--canvas)] p-2.5 transition-colors",
        isOver && "border-[var(--accent)] bg-[var(--accent-soft)]/25",
      )}
      aria-label={dateLabel(date)}
    >
      <div className="mb-2.5 flex items-center justify-between px-1">
        <h3 className="text-xs font-semibold">{dateLabel(date)}</h3>
      ...
      <div className="space-y-2">
        {shifts.map((shift) => (
          <ShiftCard
            ...
            onEdit={() => onEdit(shift)}
            onAction={(kind) => onAction(shift, kind)}
          />
        ))}
        {!shifts.length ? (
          <div className="flex min-h-24 items-center justify-center rounded-xl border border-dashed border-[var(--line)] text-center text-xs text-[var(--ink-faint)]">
            No shifts
          </div>
        ) : null}
      </div>
...
function ShiftDialog({
  ...
  return (
    <Modal
      open
      onClose={onClose}
      labelledBy="shift-dialog-title"
      initialFocusSelector="[data-shift-editor-first]"
      position="responsive-sheet"
      className="max-h-[92svh] max-w-lg overflow-y-auto rounded-b-none p-5 sm:rounded-[24px] sm:p-7"
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="eyebrow">Draft schedule</p>
          <h3
      ...
      >
        ...
        <label>
          <span className="mb-1.5 block text-xs font-semibold">Job role</span>
          <select
            name="jobRoleId"
        ...
        <label className="sm:col-span-2 flex items-center gap-2 rounded-xl bg-[var(--canvas)] px-3 py-3 text-xs">
          ...
          />
          Make this an open, unassigned shift
        </label>
        ...
        <div className="flex justify-end gap-2 sm:col-span-2">
          ...
          <Button
            type="submit"
            variant="accent"
            disabled={busy || !schedule.jobRoles.length}
          >
  ...
  return (
    <section className="mt-9">
      <SectionHeading
        ...
        title="Shift swaps"
        detail="Requests and offers remain pending until management decides"
      />
      <div className="divide-y divide-[var(--line)] border-y border-[var(--line)]">
        {pending.map((swap) => {
          const shift = shifts.find(
            (candidate) => candidate.id === swap.shiftId,
          );
          const alreadyOffered = swap.offers.some(
            (offer) => offer.employeeId === selfEmployeeId,
          );
          return (
            <article
              key={swap.id}
            ...
            >
              <div>
                ...
                <p className="mt-1 text-xs text-[var(--ink-faint)]">
                  {shift
                    ? `${shift.date} · ${shift.startLabel}–${shift.endLabel} · ${shift.jobName}`
                ...
                {swap.offers.length ? (
                  <div className="mt-3 flex flex-wrap gap-2">
                    {swap.offers.map((offer) => (
                      <StatusPill key={offer.id} tone="positive">
                        {offer.employeeName} offered
                      </StatusPill>
                    ))}
                  </div>
                ) : (
                  <p className="mt-2 text-xs text-[var(--ink-faint)]">
                    No coverage offers yet.
                  </p>
                )}
              </div>
              <div className="flex flex-wrap gap-2">
                {selfEmployeeId &&
                selfEmployeeId !== swap.requestedByEmployeeId &&
                !alreadyOffered ? (
                  <Button
                    size="sm"
                    variant="secondary"
                    disabled={busy}
                    onClick={() => onOffer(swap)}
                  >
                    Offer to cover
                  </Button>
                ) : null}
                {canManage ? (
                  <>
                    <Button
                      ...
                      variant="danger"
                      disabled={busy}
                      onClick={() => onDecide(swap, null, false)}
                    >
                      Deny
                    </Button>
                    {swap.offers.map((offer) => (
                      <Button
                        ...
                        variant="accent"
                        disabled={busy}
                        onClick={() => onDecide(swap, offer.id, true)}
                      >
                        Approve {offer.employeeName}
                      </Button>
                    ))}
                  </>
                ) : null}
        ...
        {!pending.length ? (
          <div className="px-5 py-10 text-center">
            <CircleDot className="mx-auto size-5 text-[var(--ink-faint)]" />
...
function LiveScheduleContent({
  ...
  const [shiftDialog, setShiftDialog] = useState<
    LiveScheduleShift | "new" | null
  >(null);
  const [swapDialog, setSwapDialog] = useState<LiveScheduleShift | null>(null);
  const [reopenDialog, setReopenDialog] = useState<LiveScheduleShift | null>(
    null,
  );
  ...
  function onDragEnd(event: DragEndEvent) {
    ...
    run("Shift moved.", () =>
      moveLiveShiftAction({ shiftId: shift.id, targetDate }),
    );
  }
  ...
  function shiftAction(
    ...
    if (kind === "ack")
      run("Shift acknowledged.", () =>
        acknowledgeShiftAction({ shiftId: shift.id, note: null }),
      );
    if (kind === "claim") {
      const scope = `schedule.shift.claim:${shift.id}`;
      const payload = { shiftId: shift.id };
      runStable("Open shift claimed.", scope, payload, (requestId) =>
        claimLiveOpenShiftAction({ requestId, ...payload }),
      );
    }
    if (kind === "reopen") setReopenDialog(shift);
    if (kind === "swap") setSwapDialog(shift);
  }

  function confirmReopen() {
    if (!reopenDialog) return;
    const scope = `schedule.shift.reopen:${reopenDialog.id}`;
    const payload = { shiftId: reopenDialog.id };
    runStable(
      "Shift reopened for coverage.",
      scope,
      payload,
      (requestId) => reopenLiveShiftAction({ requestId, ...payload }),
      () => setReopenDialog(null),
    );
  }

  function submitSwapRequest(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!swapDialog) return;
    const reason =
      String(new FormData(event.currentTarget).get("reason") || "").trim() ||
      null;
    const scope = `schedule.shift.swap-request:${swapDialog.id}`;
    const payload = { shiftId: swapDialog.id, reason };
    runStable(
      "Swap request sent for management review.",
      scope,
      payload,
      (requestId) => requestLiveShiftSwapAction({ requestId, ...payload }),
      () => setSwapDialog(null),
    );
  }
  ...
  function saveScheduleTemplate() {
    ...
    runStable(
      "Reusable schedule template saved.",
      scope,
      payload,
      (requestId) => saveLiveScheduleTemplateAction({ requestId, ...payload }),
    );
  }
  ...
  function saveShift(payload: Record<string, unknown>) {
    ...
    run(
      "Shift updated.",
      () => editLiveShiftAction(editPayload),
      () => setShiftDialog(null),
    );
  }

  const activeShifts = data.shifts.filter(
    (shift) => shift.status !== "cancelled",
  );
  const hours = activeShifts.reduce(
    (sum, shift) =>
      sum +
      Math.max(
        0,
  ...
  const agendaDays = data.weekDates.map((date) => {
    const shifts = data.shifts.filter((shift) => shift.date === date);
    const open = shifts.filter((shift) => shift.isOpen).length;
    return {
      id: date,
      label: dateLabel(date),
      detail: `${shifts.length - open} assigned · ${open} open`,
      items: shifts,
    };
  });
    ...
    <PageFrame width="full" className="max-w-[1680px]">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <StatusPill
              tone={published ? "positive" : draft ? "warning" : "neutral"}
              dot={published}
            >
              {published ? "Published" : draft ? "Draft" : "No schedule"}
            </StatusPill>
          ...
          <p className="mt-1 text-[13px] text-[var(--ink-faint)]">
            ...
            {data.schedule ? ` · version ${data.schedule.version}` : ""}
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          {data.canManage && draft ? (
            <Button variant="secondary" onClick={() => setShiftDialog("new")}>
              <Plus className="size-4" /> Add shift
            </Button>
          ) : null}
          {data.canManage && draft && data.shifts.length ? (
            <Button
              variant="quiet"
              disabled={isPending}
          ...
          {data.canPublish && draft ? (
            <Button
              variant="accent"
              disabled={!data.shifts.length || isPending}
              onClick={() =>
                run("Schedule published.", () =>
                  publishScheduleAction({
      ...
      <div className="mt-5 flex items-center justify-between gap-3 border-y border-[var(--line)] py-3">
        <Link
          href={`/schedule?week=${data.previousWeek}`}
          className="focus-ring inline-flex min-h-9 items-center gap-1 rounded-xl px-3 text-xs font-semibold hover:bg-[var(--canvas)]"
        >
          <ArrowLeft className="size-3.5" /> Previous
        </Link>
        <div className="text-center">
          <p className="text-xs font-semibold">
            Week of {dateLabel(data.weekStart)}
          </p>
          <p className="mt-1 text-xs text-[var(--ink-faint)]">
            {data.weekStart}–{data.weekDates[6]}
          </p>
        </div>
        <Link
      ...
      <section
        ...
        className="grid grid-cols-2 divide-x divide-y divide-[var(--line)] border-b border-[var(--line)] sm:grid-cols-4 sm:divide-y-0"
      >
        <Metric
          label="Shifts"
          value={String(activeShifts.length)}
          detail={`${activeShifts.filter((shift) => shift.isOpen).length} open`}
        />
        <Metric
          label="Scheduled hours"
          value={`${hours.toFixed(1)}h`}
          detail="After planned breaks"
        />
        <Metric
          label="Acknowledged"
          value={`${activeShifts.filter((shift) => shift.acknowledged).length}/${activeShifts.filter((shift) => !shift.isOpen).length}`}
          detail="Assigned shifts"
        />
        <Metric
          label="Swap requests"
          value={String(
            data.swaps.filter((swap) => swap.status === "pending").length,
          )}
      ...
      {!data.schedule ? (
        <section className="mt-8 rounded-[24px] border border-dashed border-[var(--line-strong)] bg-[var(--paper-strong)] p-7 text-center">
          <CalendarDays className="mx-auto size-7 text-[var(--ink-faint)]" />
          <h3 className="mt-4 text-lg font-medium tracking-[-0.035em]">
            No schedule is visible for this week
          </h3>
          <p className="mx-auto mt-2 max-w-md text-xs leading-5 text-[var(--ink-faint)]">
            {data.canManage
              ? "Start a clean draft or apply a reusable template. Nothing is published until you explicitly approve it."
              : "Management has not published a schedule in your location scope."}
          </p>
          {data.canManage ? (
            <form
              ...
              >
                ...
                {data.templates.map((template) => (
                  <option key={template.id} value={template.id}>
                    {template.name} · {template.shiftCount} shifts
                  </option>
                ))}
              </select>
        ...
        <DndContext
          id={`live-schedule-dnd-${data.schedule.id}`}
          sensors={sensors}
          collisionDetection={closestCenter}
          onDragEnd={onDragEnd}
        >
          <div className="mt-7 hidden gap-3 md:grid md:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-7">
            {data.weekDates.map((date) => (
              <DayColumn
                key={date}
                date={date}
                shifts={data.shifts.filter((shift) => shift.date === date)}
                canDrag={Boolean(data.canManage && draft)}
                canManage={data.canManage}
                selfEmployeeId={data.selfEmployeeId}
                published={published}
                busy={isPending}
                actionContext={scheduleActionContext}
                onEdit={(shift) => setShiftDialog(shift)}
                onAction={shiftAction}
              />
            ))}
          </div>
          <ScheduleAgenda
            ...
            renderItem={(shift) => (
              <ShiftCard
                shift={shift}
                draggable={false}
                editable={Boolean(data.canManage && draft)}
                canManage={data.canManage}
                selfEmployeeId={data.selfEmployeeId}
                published={published}
                busy={isPending}
                actionContext={scheduleActionContext}
                onEdit={() => setShiftDialog(shift)}
                onAction={(kind) => shiftAction(shift, kind)}
              />
            )}
          />
        </DndContext>
      ...
      {notice ? (
        <div
          ...
          role="status"
          className={cn(
            "fixed right-4 bottom-20 z-40 max-w-sm rounded-2xl px-4 py-3 text-xs shadow-lg lg:bottom-5",
            notice.tone === "success"
      ...
      {shiftDialog && data.schedule ? (
        <ShiftDialog
          schedule={data}
          shift={shiftDialog === "new" ? null : shiftDialog}
          busy={isPending}
      ...
      <ConfirmActionDialog
        open={Boolean(reopenDialog)}
        labelledBy="reopen-shift-title"
        title="Reopen this shift for coverage?"
        description={
          reopenDialog
            ? `${reopenDialog.employeeName} will be removed from the ${reopenDialog.startLabel}–${reopenDialog.endLabel} shift. The shift will return to the open coverage queue.`
            : ""
        }
      ...
      />
      <Modal
        open={Boolean(swapDialog)}
        onClose={isPending ? () => undefined : () => setSwapDialog(null)}
        labelledBy="request-swap-title"
        initialFocusSelector="[data-swap-reason]"
        position="responsive-sheet"
        ...
        <form onSubmit={submitSwapRequest}>
          <div className="border-b border-[var(--line)] px-5 py-5 sm:px-6">
            ...
            <p className="mt-2 text-sm leading-6 text-[var(--ink-faint)]">
              {swapDialog
                ? `${swapDialog.date} · ${swapDialog.startLabel}–${swapDialog.endLabel}. The shift remains assigned until management approves coverage.`
                : ""}
            </p>
          </div>
          <div className="px-5 py-5 sm:px-6">
            <label htmlFor="swap-reason" className="text-xs font-semibold">
              Reason{" "}
