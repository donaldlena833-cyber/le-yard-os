    ...
    >
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2"><p className="truncate text-xs font-semibold">{shift.employeeName}</p><StatusPill tone={shift.isOpen ? "accent" : shift.acknowledged ? "positive" : "neutral"}>{shift.isOpen ? "Open" : shift.status.replaceAll("_", " ")}</StatusPill></div>
          <p className="numeric mt-2 text-xs font-semibold">{shift.startLabel}–{shift.endLabel}</p>
          <p className="mt-1 truncate text-xs text-[var(--ink-faint)]">{shift.jobName}{shift.breakMinutes ? ` · ${shift.breakMinutes}m break` : ""}</p>
        </div>
...
function DayColumn({
  ...
  return (
    <section ref={setNodeRef} className={cn("min-h-56 rounded-[18px] border border-[var(--line)] bg-[var(--canvas)] p-2.5 transition-colors", isOver && "border-[var(--accent)] bg-[var(--accent-soft)]/25")} aria-label={dateLabel(date)}>
      <div className="mb-2.5 flex items-center justify-between px-1"><h3 className="text-xs font-semibold">{dateLabel(date)}</h3><span className="numeric text-xs text-[var(--ink-faint)]">{shifts.length}</span></div>
      <div className="space-y-2">{shifts.map((shift) => <ShiftCard key={shift.id} shift={shift} draggable={canDrag} canManage={canManage} selfEmployeeId={selfEmployeeId} published={published} busy={busy} onEdit={() => onEdit(shift)} onAction={(kind) => onAction(shift, kind)} />)}{!shifts.length ? <div className="flex min-h-24 items-center justify-center rounded-xl border border-dashed border-[var(--line)] text-center text-xs text-[var(--ink-faint)]">No shifts</div> : null}</div>
    </section>
  );
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-4 backdrop-blur-[3px]" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}>
      <section role="dialog" aria-modal="true" aria-labelledby="shift-dialog-title" className="w-full max-w-lg rounded-[24px] bg-[var(--paper-strong)] p-5 shadow-[var(--shadow-float)] sm:p-7">
        ...
        <form className="mt-6 grid gap-4 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); onSave({ scheduleId: schedule.schedule!.id, shiftId: shift?.id, employeeId: String(form.get("employeeId") || "") || null, jobRoleId: String(form.get("jobRoleId")), date: String(form.get("date")), startsAt: String(form.get("startsAt")), endsAt: String(form.get("endsAt")), breakMinutes: Number(form.get("breakMinutes") || 0), notes: String(form.get("notes") || "") || null, isOpen: form.get("isOpen") === "on" }); }}>
          <label><span className="mb-1.5 block text-xs font-semibold">Date</span><select name="date" defaultValue={shift?.date ?? schedule.weekDates[0]} className="h-11 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 text-xs">{schedule.weekDates.map((date) => <option key={date} value={date}>{dateLabel(date)}</option>)}</select></label>
          ...
          <label><span className="mb-1.5 block text-xs font-semibold">Starts</span><input name="startsAt" type="time" required defaultValue={shift?.startLocal ?? "16:00"} className="h-11 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 text-xs" /></label>
          <label><span className="mb-1.5 block text-xs font-semibold">Ends</span><input name="endsAt" type="time" required defaultValue={shift?.endLocal ?? "22:00"} className="h-11 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 text-xs" /></label>
          ...
          <label><span className="mb-1.5 block text-xs font-semibold">Break minutes</span><input name="breakMinutes" type="number" min="0" max="720" defaultValue={shift?.breakMinutes ?? 0} className="h-11 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 text-xs" /></label>
          <label className="sm:col-span-2 flex items-center gap-2 rounded-xl bg-[var(--canvas)] px-3 py-3 text-xs"><input name="isOpen" type="checkbox" defaultChecked={shift?.isOpen ?? false} className="size-4 accent-[var(--accent)]" />Make this an open, unassigned shift</label>
          <label className="sm:col-span-2"><span className="mb-1.5 block text-xs font-semibold">Notes</span><textarea name="notes" rows={3} defaultValue={shift?.notes ?? ""} className="w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 py-2 text-xs" /></label>
    ...
    <section className="mt-9">
      ...
      <div className="divide-y divide-[var(--line)] border-y border-[var(--line)]">
        {pending.map((swap) => {
          ...
          return <article key={swap.id} className="grid gap-4 py-4 lg:grid-cols-[1fr_auto] lg:items-center"><div><div className="flex flex-wrap items-center gap-2"><p className="text-xs font-semibold">{swap.requestedByName}</p><StatusPill tone="warning">Pending</StatusPill></div><p className="mt-1 text-xs text-[var(--ink-faint)]">{shift ? `${shift.date} · ${shift.startLabel}–${shift.endLabel} · ${shift.jobName}` : "Shift details unavailable"}</p><p className="mt-2 text-xs leading-4">{swap.reason || "No reason supplied."}</p>{swap.offers.length ? <div className="mt-3 flex flex-wrap gap-2">{swap.offers.map((offer) => <StatusPill key={offer.id} tone="positive">{offer.employeeName} offered</StatusPill>)}</div> : <p className="mt-2 text-xs text-[var(--ink-faint)]">No coverage offers yet.</p>}</div><div className="flex flex-wrap gap-2">{selfEmployeeId && selfEmployeeId !== swap.requestedByEmployeeId && !alreadyOffered ? <Button size="sm" variant="secondary" disabled={busy} onClick={() => onOffer(swap)}>Offer to cover</Button> : null}{canManage ? <><Button size="sm" variant="danger" disabled={busy} onClick={() => onDecide(swap, null, false)}>Deny</Button>{swap.offers.map((offer) => <Button key={offer.id} size="sm" variant="accent" disabled={busy} onClick={() => onDecide(swap, offer.id, true)}>Approve {offer.employeeName}</Button>)}</> : null}</div></article>;
        })}
        {!pending.length ? <div className="px-5 py-10 text-center"><CircleDot className="mx-auto size-5 text-[var(--ink-faint)]" /><p className="mt-3 text-xs font-semibold">No pending swap requests</p><p className="mt-1 text-xs text-[var(--ink-faint)]">New employee requests will appear here.</p></div> : null}
      </div>
    </section>
  );
}
  ...
  return (
    <PageFrame width="full" className="max-w-[1680px]">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div><div className="flex flex-wrap items-center gap-2"><StatusPill tone={published ? "positive" : draft ? "warning" : "neutral"} dot={published}>{published ? "Published" : draft ? "Draft" : "No schedule"}</StatusPill><span className="text-xs text-[var(--ink-faint)]">Live · {workspace.activeLocation.name}</span></div><h2 className="mt-3 text-2xl font-medium tracking-[-0.045em]">Schedule the week clearly</h2><p className="mt-1 text-[13px] text-[var(--ink-faint)]">Restaurant-local time · {data.timeZone}{data.schedule ? ` · version ${data.schedule.version}` : ""}</p></div>
