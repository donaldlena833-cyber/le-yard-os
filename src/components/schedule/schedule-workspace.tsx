    ...
    >
      <span className="flex items-start gap-2">
        <span className="min-w-0 flex-1">
          <span className="block truncate text-xs font-semibold">{shift.person}</span>
          <span className="mt-1 block text-xs text-[var(--ink-faint)]">{shift.role}</span>
        </span>
      ...
      {shift.hours > 6 ? <span className="mt-1 block text-xs text-[var(--ink-faint)]">30m unpaid break · manager timing</span> : null}
    </motion.button>
  );
}
    ...
    <div className="min-w-[164px] border-r border-[var(--line)] last:border-r-0">
      <div className="sticky top-0 z-10 border-b border-[var(--line)] bg-[var(--paper)] px-3 py-3">
        <div className="flex items-center justify-between">
          <span className="text-xs font-semibold text-[var(--ink-soft)]">{day.label}</span>
          <span className={cn("numeric flex size-7 items-center justify-center rounded-full text-[13px] font-semibold", day.id === "sat" ? "bg-[var(--ink)] text-[var(--paper)]" : "text-[var(--ink)]")}>{day.date}</span>
        </div>
        <p className="numeric mt-2 text-xs text-[var(--ink-faint)]">{hours}h · {shifts.length} shifts</p>
      </div>
      <div
        ref={setNodeRef}
        className={cn(
      ...
      >
        ...
        {!shifts.length ? (
          <div className="flex h-24 items-center justify-center rounded-xl border border-dashed border-[var(--line)] text-xs text-[var(--ink-faint)]">Drop a shift here</div>
        ) : null}
      </div>
    </div>
  );
  ...
  return (
    <PageFrame width="full" className="max-w-[1400px]">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div><div className="flex items-center gap-2"><StatusPill tone={published ? "positive" : "warning"} dot>{published ? "Published" : "Draft"}</StatusPill><span className="text-xs text-[var(--ink-faint)]">Le Yard · Back of house</span></div><h2 className="mt-3 text-2xl font-medium tracking-[-0.045em]">{managerMode ? "BOH schedule" : "Kitchen schedule"}</h2><p className="mt-1 text-[13px] text-[var(--ink-faint)]">Build the BOH plan, assign coverage, and publish the schedule.</p></div>
      ...
      {notice ? <p role="status" className="mt-4 rounded-xl bg-[var(--positive-soft)] px-3.5 py-3 text-xs text-[var(--positive)]">{notice}</p> : null}
      <section className="mt-7 grid gap-8 lg:grid-cols-[1.4fr_.6fr]">
        <div><SectionHeading eyebrow="Back of house" title="Aug 8–10 service plan" detail="Drag a kitchen shift between days to adjust the BOH plan." /><div className="grid gap-3 md:grid-cols-3">{[{ id: "fri", label: "Fri · Aug 8" }, { id: "sat", label: "Sat · Aug 9" }, { id: "sun", label: "Sun · Aug 10" }].map((day) => <div key={day.id} onDragOver={(event) => event.preventDefault()} onDrop={(event) => moveShift(event, day.id)} className="min-h-48 rounded-[18px] border border-dashed border-[var(--line-strong)] bg-[var(--canvas)] p-2.5"><div className="flex items-center justify-between px-1.5 pb-2"><p className="text-xs font-semibold">{day.label}</p><span className="text-xs text-[var(--ink-faint)]">Drop here</span></div>{shifts.filter((shift) => shift.dayId === day.id).map((shift) => <div key={shift.id} draggable onDragStart={() => setDraggedId(shift.id)} onDragEnd={() => setDraggedId(null)} className="mb-2 cursor-grab rounded-[14px] border border-[var(--line)] bg-[var(--paper-strong)] p-3 shadow-sm active:cursor-grabbing"><div className="flex items-start justify-between gap-2"><p className="text-[13px] font-semibold">{shift.role}</p><span className="text-xs text-[var(--ink-faint)]">Drag</span></div><p className="mt-1 text-xs text-[var(--ink-faint)]">{shift.time}</p><p className="mt-2 text-xs text-[var(--ink-faint)]">{shift.people}</p><p className="mt-2 text-xs text-[var(--ink-faint)]">30m unpaid break · manager timing</p><div className="mt-2"><StatusPill tone={shift.status === "Ready" ? "positive" : "warning"}>{shift.status}</StatusPill></div></div>)}</div>)}</div></div>
        <aside><SectionHeading eyebrow="Coverage" title="Kitchen staffing" detail="Current BOH coverage before publishing." /><div className="border-y border-[var(--line)]"><div className="flex items-center justify-between border-t border-[var(--line)] px-3 py-4 first:border-0"><span className="text-[13px] text-[var(--ink-faint)]">Line coverage</span><StatusPill tone="positive">2 / 2</StatusPill></div><div className="flex items-center justify-between border-t border-[var(--line)] px-3 py-4"><span className="text-[13px] text-[var(--ink-faint)]">Prep coverage</span><StatusPill tone="warning">1 open</StatusPill></div><div className="flex items-center justify-between border-t border-[var(--line)] px-3 py-4"><span className="text-[13px] text-[var(--ink-faint)]">Recipes to review</span><StatusPill tone="neutral">2</StatusPill></div></div></aside>
      </section>
    </PageFrame>
  );
}
...
function EmployeeScheduleWorkspace() {
  ...
  return (
    <PageFrame width="full" className="max-w-[1400px]">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div><div className="flex items-center gap-2"><StatusPill tone="positive" dot>Published</StatusPill><span className="text-xs text-[var(--ink-faint)]">Le Yard · Aug 3–9</span></div><h2 className="mt-3 text-2xl font-medium tracking-[-0.045em]">Your schedule</h2><p className="mt-1 text-[13px] text-[var(--ink-faint)]">See your shifts, offer coverage, and tell managers when you are unavailable.</p></div>
    ...
    <PageFrame width="full" className="max-w-[1700px]">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-1 text-[13px] text-[var(--ink-faint)]">Drag shifts between days. Publishing creates a new auditable version.</p>
        </div>
          ...
          <motion.div className="fixed inset-0 z-50 bg-black/20 backdrop-blur-[2px]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => { if (event.target === event.currentTarget) setSelected(null); }}>
            <motion.aside className="absolute inset-y-0 right-0 w-[min(92vw,430px)] overflow-y-auto bg-[var(--paper-strong)] p-5 shadow-2xl sm:p-7" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", stiffness: 360, damping: 36 }}>
              ...
              <div className="mt-7 flex items-center gap-3 border-y border-[var(--line)] py-4">
                ...
                <div><p className="text-sm font-semibold">{selected.role}</p><p className="numeric mt-1 text-[13px] text-[var(--ink-faint)]">{selected.start}–{selected.end} · {selected.hours} hours</p></div>
              </div>
              ...
              {!selected.acknowledged ? (
                <div className="mt-8 rounded-[16px] bg-[var(--accent-soft)]/50 p-4">
                  ...
                  <p className="mt-1 text-xs leading-4 text-[var(--ink-faint)]">Use this to verify the acknowledgement flow in demo mode.</p>
          ...
          <motion.div className="fixed inset-0 z-50 flex items-center justify-center bg-black/25 px-4 backdrop-blur-[3px]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => { if (event.target === event.currentTarget) setAddOpen(false); }}>
            <motion.div role="dialog" aria-modal="true" aria-label="Add shift" className="w-full max-w-lg rounded-[22px] bg-[var(--paper-strong)] p-5 shadow-[var(--shadow-float)] sm:p-6" initial={{ y: 12, scale: .98 }} animate={{ y: 0, scale: 1 }} exit={{ y: 8, scale: .98 }}>
              ...
              <form action={addShift} className="mt-6 grid gap-4 sm:grid-cols-2">
                ...
                <label><span className="mb-1.5 block text-xs font-semibold">Day</span><select name="dayId" className="h-11 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 text-xs">{days.map((day) => <option key={day.id} value={day.id}>{day.label}, Aug {day.date}</option>)}</select></label>
                <label><span className="mb-1.5 block text-xs font-semibold">Role</span><select name="role" className="h-11 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 text-xs">{Object.keys(roleTone).map((role) => <option key={role}>{role}</option>)}</select></label>
                <label><span className="mb-1.5 block text-xs font-semibold">Starts</span><input name="start" defaultValue="5:00p" className="h-11 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 text-xs" /></label>
                <label><span className="mb-1.5 block text-xs font-semibold">Ends</span><input name="end" defaultValue="11:00p" className="h-11 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 text-xs" /></label>
