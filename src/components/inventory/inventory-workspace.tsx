...
export function InventoryWorkspace() {
  ...
  const latestCount = demoWorkspace.inventoryCounts.find((count) => count.locationId === locationId)!;
  const [activeTab, setActiveTab] = useState<Tab>("stock");
  const [query, setQuery] = useState("");
  const [countValues, setCountValues] = useState<Record<string, number>>(
    Object.fromEntries(latestCount.lines.map((line) => [line.itemId, line.countedQuantity])),
  );
  const [countSubmitted, setCountSubmitted] = useState(false);
  const [waste, setWaste] = useState<WasteRecord[]>(demoWorkspace.wasteRecords);
  const [wasteOpen, setWasteOpen] = useState(false);

  const stockRows = useMemo(
    () =>
      demoWorkspace.inventoryItems
        .filter((item) => item.name.toLowerCase().includes(query.trim().toLowerCase()))
        .map((item) => {
          const line = latestCount.lines.find((candidate) => candidate.itemId === item.id);
    ...
    [latestCount.lines, locationId, query],
  );
  ...
  function addWaste(formData: FormData) {
    ...
    const item = demoWorkspace.inventoryItems.find((candidate) => candidate.id === itemId)!;
    ...
    <PageFrame>
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-1 text-[11px] text-[var(--ink-faint)]">{workspace.activeLocation.name} · Last approved mock count Jul 31</p>
        </div>
      ...
      <section className="mt-5 grid grid-cols-2 divide-x divide-y divide-[var(--line)] border-y border-[var(--line)] sm:grid-cols-4 sm:divide-y-0">
        ...
        <Metric label="Open orders" value={String(demoWorkspace.purchaseOrders.filter((order) => !["received", "cancelled"].includes(order.status)).length)} detail="Next delivery Aug 2" />
        <Metric label="Waste · 7d" value={formatMoney(waste.reduce((sum, record) => sum + record.valueCents, 0))} detail="0.8% of food purchases" trend={{ label: "−0.2pt", tone: "positive" }} />
      </section>

      <div className="mt-6 flex items-center gap-1 overflow-x-auto border-b border-[var(--line)]">
        {tabs.map((tab) => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={cn("focus-ring relative min-h-10 shrink-0 px-3 text-[11px] font-semibold transition-colors", activeTab === tab.id ? "text-[var(--ink)]" : "text-[var(--ink-faint)] hover:text-[var(--ink-soft)]")}>
            {tab.label}
        ...
        <section className="mt-5">
          ...
          <div className="overflow-x-auto border-y border-[var(--line)]">
            ...
            {stockRows.map(({ item, onHand, par, reorder }) => {
              ...
              return <button key={item.id} className="focus-ring grid min-w-[720px] w-full grid-cols-[1.35fr_.6fr_.6fr_.65fr_.7fr] items-center gap-4 border-t border-[var(--line)] px-4 py-3.5 text-left hover:bg-[var(--paper)]"><span className="flex items-center gap-3"><span className="flex size-8 items-center justify-center rounded-xl bg-[var(--canvas-strong)]"><Boxes className="size-3.5 text-[var(--ink-faint)]" /></span><span><span className="block text-xs font-semibold">{item.name}</span><span className="mt-1 block text-[9px] text-[var(--ink-faint)]">{item.sku} · {item.category}</span></span></span><span className="numeric text-xs font-semibold">{onHand} <span className="font-normal text-[var(--ink-faint)]">{item.baseUnit}</span></span><span className="numeric text-[10px] text-[var(--ink-faint)]">{par}</span><span><StatusPill tone={status === "healthy" ? "positive" : status === "reorder" ? "danger" : "warning"}>{status === "healthy" ? "Healthy" : status === "reorder" ? "Reorder" : "Below par"}</StatusPill></span><span className="numeric text-right text-xs">{formatMoney(item.lastUnitCostCents)} / {item.baseUnit}</span></button>;
            })}
          </div>
        </section>
      ) : null}
      ...
      {activeTab === "count" ? (
        <section className="mt-5">
          ...
          <div className="mt-4 overflow-x-auto border-y border-[var(--line)]">
            ...
            {demoWorkspace.inventoryItems.map((item) => {
              const line = latestCount.lines.find((candidate) => candidate.itemId === item.id);
              const expected = line?.expectedQuantity || 0;
              const counted = countValues[item.id] ?? 0;
              const variance = counted - expected;
              return <div key={item.id} className="grid min-w-[680px] grid-cols-[1.3fr_.6fr_.65fr_.65fr] items-center gap-4 border-t border-[var(--line)] px-4 py-3"><div><p className="text-xs font-semibold">{item.name}</p><p className="mt-1 text-[9px] text-[var(--ink-faint)]">{item.baseUnit}</p></div><span className="numeric text-xs text-[var(--ink-faint)]">{expected}</span><input aria-label={`Counted ${item.name}`} type="number" step="0.1" min="0" value={counted} disabled={countSubmitted} onChange={(event) => { setCountValues((current) => ({ ...current, [item.id]: Number(event.target.value) })); setCountSubmitted(false); }} className="h-9 w-24 rounded-lg border border-[var(--line)] bg-[var(--paper-strong)] px-3 text-right text-xs font-semibold outline-none focus:border-[var(--accent)] disabled:opacity-60" /><span className={cn("numeric flex items-center gap-1 text-xs font-semibold", variance < 0 ? "text-[var(--danger)]" : variance > 0 ? "text-[var(--positive)]" : "text-[var(--ink-faint)]")}>{variance < 0 ? <ArrowDown className="size-3" /> : variance > 0 ? <ArrowUp className="size-3" /> : null}{variance.toFixed(1)}</span></div>;
            })}
          </div>
          <div className="mt-5 flex justify-end gap-2"><Button variant="secondary" onClick={() => setCountValues(Object.fromEntries(latestCount.lines.map((line) => [line.itemId, line.countedQuantity])))}>Reset</Button><Button variant="accent" disabled={countSubmitted} onClick={() => setCountSubmitted(true)}><Check className="size-4" /> {countSubmitted ? "Submitted" : "Submit count"}</Button></div>
        </section>
      ) : null}

      {activeTab === "orders" ? (
        <section className="mt-5"><SectionHeading title="Purchase orders" detail="Create, receive, and match orders to invoices." action={<Button variant="accent" size="sm"><Plus className="size-3.5" /> New order</Button>} /><div className="border-y border-[var(--line)]">{demoWorkspace.purchaseOrders.map((order) => { const vendor = demoWorkspace.vendors.find((item) => item.id === order.vendorId); return <button key={order.id} className="focus-ring flex w-full items-center gap-4 border-t border-[var(--line)] px-4 py-4 text-left first:border-0 hover:bg-[var(--paper)]"><span className="flex size-9 items-center justify-center rounded-xl bg-[var(--canvas-strong)]"><Truck className="size-4 text-[var(--ink-faint)]" /></span><span className="min-w-0 flex-1"><span className="block text-xs font-semibold">{vendor?.name}</span><span className="mt-1 block text-[9px] text-[var(--ink-faint)]">{order.orderNumber} · Expected {order.expectedOn} · {order.lines.length} lines</span></span><span className="numeric text-xs font-semibold">{formatMoney(order.totalCents)}</span><StatusPill tone={order.status === "received" ? "positive" : "warning"}>{order.status}</StatusPill></button>; })}</div></section>
      ) : null}

      {activeTab === "vendors" ? (
        <section className="mt-5"><SectionHeading title="Vendors" detail="Contacts, terms, and recent price movement." /> <div className="grid gap-x-8 gap-y-1 md:grid-cols-2">{demoWorkspace.vendors.map((vendor) => <div key={vendor.id} className="flex items-start gap-3 border-b border-[var(--line)] py-4"><span className="flex size-9 items-center justify-center rounded-xl bg-[var(--canvas-strong)]"><PackageOpen className="size-4 text-[var(--ink-faint)]" /></span><div className="min-w-0 flex-1"><p className="text-xs font-semibold">{vendor.name}</p><p className="mt-1 text-[9px] text-[var(--ink-faint)]">{vendor.contactName} · {vendor.paymentTerms}</p><p className="mt-2 text-[10px] text-[var(--ink-soft)]">{vendor.email}</p></div><StatusPill tone="positive">Active</StatusPill></div>)}</div></section>
      ) : null}

      {activeTab === "recipes" ? (
        <section className="mt-5"><SectionHeading title="Recipe costing" detail="Current ingredient costs compared with menu price." /><div className="border-y border-[var(--line)]">{demoWorkspace.recipes.map((recipe) => <div key={recipe.id} className="grid grid-cols-[1fr_auto] gap-3 border-t border-[var(--line)] px-4 py-4 first:border-0 sm:grid-cols-[1.2fr_.5fr_.5fr_.5fr]"><div><p className="text-xs font-semibold">{recipe.name}</p><p className="mt-1 text-[9px] text-[var(--ink-faint)]">{recipe.menuCode} · {recipe.ingredients.length} ingredients</p></div><div className="hidden sm:block"><p className="text-[9px] text-[var(--ink-faint)]">Cost</p><p className="numeric mt-1 text-xs font-semibold">{formatMoney(recipe.costPerYieldCents)}</p></div><div className="hidden sm:block"><p className="text-[9px] text-[var(--ink-faint)]">Menu price</p><p className="numeric mt-1 text-xs font-semibold">{formatMoney(recipe.menuPriceCents)}</p></div><div className="text-right sm:text-left"><p className="text-[9px] text-[var(--ink-faint)]">Food cost</p><p className={cn("numeric mt-1 text-xs font-semibold", recipe.foodCostPercentage > 30 ? "text-[var(--warning)]" : "text-[var(--positive)]")}>{recipe.foodCostPercentage.toFixed(1)}%</p></div></div>)}</div></section>
      ) : null}

      {activeTab === "waste" ? (
        <section className="mt-5"><SectionHeading title="Waste log" detail="Human-entered adjustments; AI may suggest but never posts them." action={<Button variant="accent" size="sm" onClick={() => setWasteOpen(true)}><Plus className="size-3.5" /> Record waste</Button>} /><div className="border-y border-[var(--line)]">{waste.map((record) => { const item = demoWorkspace.inventoryItems.find((candidate) => candidate.id === record.itemId); return <div key={record.id} className="flex items-center gap-4 border-t border-[var(--line)] px-4 py-4 first:border-0"><span className="flex size-9 items-center justify-center rounded-xl bg-[var(--danger-soft)] text-[var(--danger)]"><Trash2 className="size-4" /></span><div className="min-w-0 flex-1"><p className="text-xs font-semibold">{item?.name}</p><p className="mt-1 text-[9px] text-[var(--ink-faint)]">{record.quantity} {record.unit} · {record.reason.replaceAll("_", " ")} · {new Date(record.occurredAt).toLocaleString()}</p></div><span className="numeric text-xs font-semibold">{formatMoney(record.valueCents)}</span></div>; })}</div></section>
      ) : null}

      <div className="mt-7 flex items-start gap-3 rounded-[16px] bg-[var(--accent-soft)]/50 px-4 py-3 text-[10px] leading-4 text-[var(--accent-strong)]"><Sparkles className="mt-0.5 size-4 shrink-0" /><span><strong>Price watch:</strong> Roma tomatoes increased 8.6% since June. This insight cites two verified invoice records and does not change the preferred vendor automatically.</span></div>
          ...
          <motion.div className="fixed inset-0 z-50 flex items-center justify-center bg-black/25 px-4 backdrop-blur-[3px]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => { if (event.target === event.currentTarget) setWasteOpen(false); }}>
            <motion.div role="dialog" aria-modal="true" aria-label="Record waste" className="w-full max-w-lg rounded-[22px] bg-[var(--paper-strong)] p-5 shadow-[var(--shadow-float)] sm:p-6" initial={{ y: 12, scale: .98 }} animate={{ y: 0, scale: 1 }} exit={{ y: 8, scale: .98 }}>
              ...
              <form action={addWaste} className="mt-6 grid gap-4 sm:grid-cols-2">
                <label className="sm:col-span-2"><span className="mb-1.5 block text-[10px] font-semibold">Item</span><select name="itemId" className="h-11 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 text-xs">{demoWorkspace.inventoryItems.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
                <label><span className="mb-1.5 block text-[10px] font-semibold">Quantity</span><input required name="quantity" type="number" min="0.1" step="0.1" defaultValue="1" className="h-11 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 text-xs" /></label>
                <label><span className="mb-1.5 block text-[10px] font-semibold">Unit</span><select name="unit" className="h-11 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 text-xs"><option>ounce</option><option>pound</option><option>each</option><option>liter</option></select></label>
                ...
                <label className="sm:col-span-2"><span className="mb-1.5 block text-[10px] font-semibold">Note</span><textarea name="note" rows={3} className="w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] p-3 text-xs" /></label>
