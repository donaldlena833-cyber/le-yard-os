...
export function InventoryCatalogWorkspace({ model, workspace }: { model: LiveInventoryModel; workspace: WorkspaceContextValue }) {
  ...
  const canConfigureRecipes = canConfigure || workspace.role === "manager";
  ...
  const action = (label: string, kind: CatalogDialog["kind"], disabled = false, allowed = canConfigure) => allowed ? <Button size="sm" variant="secondary" disabled={disabled} onClick={() => open(kind)}><Plus className="size-3.5" />{label}</Button> : null;
  ...
  return (
    <section className="mt-5">
      ...
      {!canConfigure ? <div className="mb-5 flex items-start gap-3 rounded-xl bg-[var(--warning-soft)] px-4 py-3 text-[10px] leading-5 text-[var(--warning)]"><CircleAlert className="mt-0.5 size-4 shrink-0" />{workspace.role === "manager" ? "Managers can edit recipes only. Owners and admins maintain units, vendors, items, prices, and location pars." : roleAllows ? "Owner changes require a current multi-factor session." : "Only Owners and Admins can change inventory setup."}</div> : null}
      ...
      <SetupStep number="07" icon={<UtensilsCrossed className="size-4" />} title="Recipes & ingredients" detail="Recipes version on every save, preserving the exact ingredient snapshot behind historical costing." action={action("Recipe", "recipe", !catalog.items.some((item) => item.isActive) || !catalog.units.some((unit) => unit.isActive), canConfigureRecipes)}>
