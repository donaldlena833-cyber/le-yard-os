    ...
    <PageFrame>
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-1 text-[11px] text-[var(--ink-faint)]">Private files · signed access · human-reviewed extraction</p>
        </div>
      ...
      <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2 text-[10px] text-[var(--ink-faint)]"><Sparkles className="size-3.5 text-[var(--accent)]" /> Search reads extracted text, not only filenames.</div>
      </div>

      <section className="mt-5 overflow-hidden border-y border-[var(--line)]">
        <div className="hidden grid-cols-[1.25fr_.8fr_.7fr_.55fr_36px] gap-4 bg-[var(--canvas-strong)] px-4 py-2.5 text-[9px] font-semibold tracking-[0.12em] text-[var(--ink-faint)] uppercase sm:grid">
          ...
          return (
            <button key={receipt.id} onClick={() => setSelected(receipt)} className="focus-ring grid w-full grid-cols-[1fr_auto] items-center gap-3 border-t border-[var(--line)] px-4 py-4 text-left first:border-0 transition-colors hover:bg-[var(--paper)] sm:grid-cols-[1.25fr_.8fr_.7fr_.55fr_36px] sm:gap-4">
              <span className="flex min-w-0 items-center gap-3">
                ...
                <span className="min-w-0"><span className="block truncate text-xs font-semibold">{vendor?.name || "Unmatched vendor"}</span><span className="mt-1 block truncate text-[9px] text-[var(--ink-faint)]">{receipt.documentNumber} · {receipt.file.fileName}</span></span>
              </span>
              <span className="numeric text-right text-xs font-semibold sm:hidden">{formatMoney(receipt.totalCents)}</span>
              <span className="hidden text-[10px] text-[var(--ink-faint)] sm:block"><span className="block text-[var(--ink-soft)]">{receipt.documentDate}</span><span className="mt-1 capitalize">{receipt.expenseCategory.replaceAll("_", " ")}</span></span>
        ...
        {!filtered.length ? <div className="px-5 py-12 text-center"><FileSearch className="mx-auto size-6 text-[var(--ink-faint)]" /><p className="mt-3 text-xs font-semibold">No matching documents</p><p className="mt-1 text-[10px] text-[var(--ink-faint)]">Try a vendor, amount, or phrase from the receipt.</p></div> : null}
      </section>

      <div className="mt-5 flex items-start gap-3 rounded-[16px] bg-[var(--positive-soft)] px-4 py-3 text-[10px] leading-4 text-[var(--positive)]">
        ...
        {selected ? (
          <motion.div className="fixed inset-0 z-50 bg-black/20 backdrop-blur-[2px]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => { if (event.target === event.currentTarget) setSelected(null); }}>
            <motion.aside className="absolute inset-y-0 right-0 w-[min(94vw,520px)] overflow-y-auto bg-[var(--paper-strong)] p-5 pb-[calc(6.5rem+env(safe-area-inset-bottom))] shadow-2xl sm:p-7" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", stiffness: 350, damping: 35 }}>
              <div className="flex items-start justify-between gap-4"><div><p className="eyebrow">Extraction review</p><h3 className="mt-3 text-xl font-medium tracking-[-0.04em]">{demoWorkspace.vendors.find((item) => item.id === selected.vendorId)?.name || "Unmatched vendor"}</h3><p className="mt-1 text-[10px] text-[var(--ink-faint)]">{selected.file.fileName}</p></div><Button variant="quiet" size="icon" onClick={() => setSelected(null)}><X className="size-4" /></Button></div>
              <div className="mt-6 flex items-center gap-3 rounded-[15px] bg-[var(--accent-soft)]/55 px-4 py-3"><Sparkles className="size-4 text-[var(--accent-strong)]" /><div className="flex-1"><p className="text-[10px] font-semibold">AI extraction · {(selected.extractionConfidence * 100).toFixed(0)}% confidence</p><p className="mt-1 text-[9px] text-[var(--ink-faint)]">Review required before posting</p></div><StatusPill tone={selected.reviewStatus === "verified" ? "positive" : "warning"}>{selected.reviewStatus === "verified" ? "Verified" : "Draft"}</StatusPill></div>
