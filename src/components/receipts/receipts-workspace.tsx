...
export function ReceiptsWorkspace() {
  ...
  const [receipts, setReceipts] = useState<UiReceipt[]>(
    demoWorkspace.receipts.filter(
      (receipt) => receipt.locationId === workspace.activeLocation.id,
    ),
  );
  ...
  const filtered = useMemo(() => {
    ...
    return receipts.filter((receipt) => {
      const vendor =
        demoWorkspace.vendors.find((item) => item.id === receipt.vendorId)
          ?.name || "";
      return `${vendor} ${receipt.documentNumber} ${receipt.ocrText} ${receipt.expenseCategory}`
        .toLowerCase()
        .includes(needle);
    });
  }, [query, receipts]);
  ...
  const needsReview = receipts.filter(
    (receipt) => receipt.reviewStatus === "needs_review",
  );
  ...
  return (
    <PageFrame>
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-1 text-[13px] text-[var(--ink-faint)]">
            Private files · signed access · human-reviewed extraction
          </p>
        </div>
      ...
      {notice ? (
        <p
          role="status"
          className="mt-3 rounded-xl bg-[var(--canvas-strong)] px-3 py-2 text-xs text-[var(--ink-soft)]"
        >
          {notice}
        </p>
      ) : null}
      ...
      <section className="mt-5 grid grid-cols-2 divide-x divide-y divide-[var(--line)] border-y border-[var(--line)] sm:grid-cols-4 sm:divide-y-0">
        <Metric
          ...
          detail={`${receipts.length} documents`}
        />
        <Metric
          label="Needs review"
          value={String(needsReview.length)}
          detail="AI cannot self-approve"
          trend={{
            label: needsReview.length ? "Action" : "Clear",
            tone: needsReview.length ? "negative" : "positive",
          }}
        />
        <Metric
          ...
          value={
            receipts.length
              ? `${Math.round((receipts.reduce((sum, receipt) => sum + receipt.extractionConfidence, 0) / receipts.length) * 100)}%`
              : "—"
          }
          detail={
            receipts.length
        ...
        <Metric
          ...
          value={String(
            receipts.filter((receipt) => receipt.duplicateOfId).length,
          )}
          detail="Matched by hash + amount"
        />
      </section>
      ...
      <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2 text-xs text-[var(--ink-faint)]">
          <Sparkles className="size-3.5 text-[var(--accent)]" /> Search reads
          extracted text, not only filenames.
        </div>
      </div>
      ...
      <section className="mt-5 overflow-hidden border-y border-[var(--line)]">
        ...
        {filtered.map((receipt) => {
          const vendor = demoWorkspace.vendors.find(
            (item) => item.id === receipt.vendorId,
          );
          return (
            <button
              key={receipt.id}
              ...
              className="focus-ring grid w-full grid-cols-[1fr_auto] items-center gap-3 border-t border-[var(--line)] px-4 py-4 text-left first:border-0 transition-colors hover:bg-[var(--paper)] sm:grid-cols-[1.25fr_.8fr_.7fr_.55fr_36px] sm:gap-4"
            >
              <span className="flex min-w-0 items-center gap-3">
                <span className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-[var(--canvas-strong)] text-[var(--ink-soft)]">
                  <ReceiptText className="size-4" />
                </span>
                <span className="min-w-0">
                  <span className="block truncate text-xs font-semibold">
                    {vendor?.name || "Unmatched vendor"}
                  </span>
                  <span className="mt-1 block truncate text-xs text-[var(--ink-faint)]">
                    {receipt.documentNumber} · {receipt.file.fileName}
                  </span>
                </span>
              </span>
              <span className="numeric text-right text-xs font-semibold sm:hidden">
              ...
              <span className="hidden text-xs text-[var(--ink-faint)] sm:block">
                ...
                <span className="mt-1 capitalize">
                  {receipt.expenseCategory.replaceAll("_", " ")}
                </span>
              </span>
              <span className="numeric hidden text-xs font-semibold sm:block">
                {formatMoney(receipt.totalCents)}
              </span>
              <span className="hidden sm:block">
                <StatusPill
                  tone={
                    receipt.reviewStatus === "verified" ? "positive" : "warning"
                  }
                  dot
                >
                  {receipt.reviewStatus === "verified" ? "Verified" : "Review"}
                </StatusPill>
              </span>
        ...
        {!filtered.length ? (
          <div className="px-5 py-12 text-center">
            <FileSearch className="mx-auto size-6 text-[var(--ink-faint)]" />
            <p className="mt-3 text-xs font-semibold">No matching documents</p>
            <p className="mt-1 text-xs text-[var(--ink-faint)]">
              Try a vendor, amount, or phrase from the receipt.
            </p>
          </div>
        ) : null}
      </section>
      ...
      <div className="mt-5 flex items-start gap-3 rounded-[16px] bg-[var(--positive-soft)] px-4 py-3 text-xs leading-4 text-[var(--positive)]">
        ...
        Files are designed for private Supabase Storage buckets. Production
        downloads use short-lived signed URLs and every review is written to the
        audit log.
      </div>
      ...
      <AnimatePresence>
        {processing ? (
          <motion.div
            ...
            exit={{ opacity: 0 }}
          >
            <div className="w-full max-w-sm rounded-[22px] bg-[var(--paper-strong)] p-7 text-center shadow-[var(--shadow-float)]">
              <LoaderCircle className="mx-auto size-6 animate-spin text-[var(--accent)]" />
              <p className="mt-4 text-sm font-semibold">Reading document</p>
              <p className="mt-2 text-xs leading-4 text-[var(--ink-faint)]">
                Extracting vendor, date, totals, category, and duplicate
                fingerprint…
              </p>
            </div>
          </motion.div>
      ...
      <Drawer
        ...
        labelledBy="demo-receipt-review-title"
        width="lg"
        initialFocusSelector="[data-close-demo-receipt]"
          ...
          <>
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h3
                  id="demo-receipt-review-title"
                  className="mt-3 text-xl font-medium tracking-[-0.04em]"
                >
                  {demoWorkspace.vendors.find(
                    (item) => item.id === selected.vendorId,
                  )?.name || "Unmatched vendor"}
                </h3>
                <p className="mt-1 text-xs text-[var(--ink-faint)]">
            ...
            <div className="mt-6 flex items-center gap-3 rounded-[15px] bg-[var(--accent-soft)]/55 px-4 py-3">
              <Sparkles className="size-4 text-[var(--accent-strong)]" />
              <div className="flex-1">
                <p className="text-xs font-semibold">
                  AI extraction ·{" "}
                  {(selected.extractionConfidence * 100).toFixed(0)}% confidence
                </p>
                <p className="mt-1 text-xs text-[var(--ink-faint)]">
              ...
              <StatusPill
                tone={
                  selected.reviewStatus === "verified" ? "positive" : "warning"
                }
              >
                {selected.reviewStatus === "verified" ? "Verified" : "Draft"}
              </StatusPill>
            </div>
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              {[
                ...
                { label: "Document date", value: selected.documentDate },
                {
                  label: "Subtotal",
                  value: (selected.subtotalCents / 100).toFixed(2),
                },
                { label: "Tax", value: (selected.taxCents / 100).toFixed(2) },
                {
                  label: "Total",
                  value: (selected.totalCents / 100).toFixed(2),
                },
                {
                  label: "Category",
                  value: selected.expenseCategory.replaceAll("_", " "),
                },
              ].map((field) => (
            ...
            <div className="mt-6">
              <p className="text-xs font-semibold">Recognized text</p>
              <p className="mt-2 rounded-xl bg-[var(--canvas)] p-3 font-mono text-xs leading-4 text-[var(--ink-faint)]">
                {selected.ocrText}
              </p>
            </div>
            <InvoiceInventoryMatches text={selected.ocrText} catalog={[]} />
            {selected.duplicateOfId ? (
              <div className="mt-5 flex gap-3 rounded-xl bg-[var(--warning-soft)] p-3 text-xs text-[var(--warning)]">
                <CircleAlert className="size-4 shrink-0" />
                Possible duplicate of {selected.duplicateOfId}. Verify before
                posting.
              </div>
            ) : null}
            <div className="mt-6 flex flex-wrap items-center gap-2 border-t border-[var(--line)] pt-5">
              <Button
                ...
                onClick={() =>
                  setNotice(
                    "Purchase linking will be enabled when the live inventory catalog is connected.",
                  )
                }
              >
                <Link2 className="size-3.5" /> Link purchase
              </Button>
              <Button
                variant="quiet"
                onClick={() =>
                  setNotice(
                    "This local preview file is held in the browser until private storage is connected.",
                  )
                }
              >
              ...
              <Button
                className="ml-auto"
                variant="accent"
                disabled={selected.reviewStatus === "verified"}
                onClick={() => verifyReceipt(selected)}
              >
