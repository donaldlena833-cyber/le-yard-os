...
function ReceiptInspector({
  ...
  const { requestIdFor, rotateRequestId, rotateAllRequestIds } =
    useStableRequestIds();
  ...
  async function review(status: ReviewStatus) {
    ...
    setMessage(
      status === "approved"
        ? "Receipt approved and locked."
        : `Receipt marked ${status.replaceAll("_", " ")}.`,
    );
    router.refresh();
  }
  ...
  async function openFile(file: LiveReceiptRow["files"][number]) {
    ...
    if (!result.ok || !("data" in result)) {
      setMessage(
        result.ok ? "The private file is unavailable." : result.message,
      );
      return;
    }
  ...
  async function linkReference(
    ...
    setMessage(
      receiptId
        ? `${kind === "expense" ? "Expense" : "Delivery"} linked.`
        : "Receipt link removed.",
    );
    rotateRequestId(scope);
    router.refresh();
  }
  ...
  async function assignInventoryMatch(input: {
    ...
    rotateRequestId(scope);
    setMessage(
      "Invoice line assigned to inventory. Review it with the receipt before posting a delivery.",
    );
    return true;
  }

  const terminal =
    receipt.reviewStatus === "approved" || receipt.reviewStatus === "rejected";
  const unresolvedMatches = receipt.duplicateMatches.filter(
  ...
  const linkedDeliveries = deliveries.filter(
    (delivery) => delivery.receiptId === receipt.id,
  );
  ...
  return (
    <Drawer
      ...
      labelledBy="receipt-review-title"
      width="lg"
      initialFocusSelector="[data-close-receipt]"
      returnFocusTarget={returnFocusTarget}
      className="p-5 pb-[calc(6.5rem+env(safe-area-inset-bottom))] sm:p-8"
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <StatusPill
              tone={statusTone[receipt.reviewStatus] ?? "neutral"}
              dot
            >
              {receipt.reviewStatus.replaceAll("_", " ")}
            </StatusPill>
            {receipt.duplicateCount ? (
              <StatusPill tone="warning">
                {receipt.duplicateCount} possible duplicate
              </StatusPill>
            ) : null}
          </div>
          <h3
      ...
      <form
        ...
        onSubmit={(event) => {
          ...
          void review("in_review");
        }}
      >
        <div className="grid gap-4 sm:grid-cols-2">
          <label>
            <span className="mb-1.5 block text-xs font-semibold">Vendor</span>
            <select
              name="vendorId"
          ...
          <label>
            <span className="mb-1.5 block text-xs font-semibold">Total</span>
            <input
              name="total"
              type="number"
              min="0"
          ...
          <label>
            <span className="mb-1.5 block text-xs font-semibold">Tax</span>
            <input
              name="tax"
              type="number"
              min="0"
        ...
        <section className="mt-7 border-t border-[var(--line)] pt-6">
          <SectionHeading
            ...
            detail="Resolve every possible match before approval"
          />
          <div className="mt-3 space-y-2">
            {receipt.duplicateMatches.map((match) => (
              <div
                key={match.id}
                className="rounded-xl bg-[var(--canvas)] p-3.5"
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="flex items-center gap-2 text-xs font-semibold">
                      <GitCompare className="size-3.5 text-[var(--warning)]" />
                      Receipt {match.possibleDuplicateId.slice(0, 8)}
                    </p>
                    <p className="numeric mt-1 text-xs text-[var(--ink-faint)]">
                      ...
                      {match.reasons.length
                        ? match.reasons
                            ...
                            .join(", ")
                        : "No machine reason recorded"}
                    </p>
                  </div>
                  {match.resolution ? (
                    <StatusPill
                      tone={
                        match.resolution === "duplicate"
                          ? "warning"
                          : "positive"
                      }
                    >
                      {match.resolution.replaceAll("_", " ")}
                    </StatusPill>
                  ) : (
                    <div className="flex gap-2">
                      <Button
                        ...
                        disabled={busy}
                        onClick={() =>
                          void resolveDuplicate(match.id, "not_duplicate")
                        }
                      >
                        Separate
                      </Button>
                      <Button
                        ...
                        disabled={busy}
                        onClick={() =>
                          void resolveDuplicate(match.id, "duplicate")
                        }
                      >
                        Duplicate
                      </Button>
            ...
            {!receipt.duplicateMatches.length ? (
              <p className="rounded-xl bg-[var(--positive-soft)] px-3.5 py-3 text-xs text-[var(--positive)]">
                No possible duplicate evidence is recorded for this document.
              </p>
            ) : null}
          </div>
        </section>
        ...
        <section className="mt-7 border-t border-[var(--line)] pt-6">
          <SectionHeading
            title="Private files"
            detail="Signed access expires after 60 seconds"
          />
          <div className="mt-3 space-y-2">
            {receipt.files.map((file) => (
              <button
                ...
                className="focus-ring flex w-full items-center gap-3 rounded-xl bg-[var(--canvas)] px-3.5 py-3 text-left"
              >
                <ReceiptText className="size-4 text-[var(--accent-strong)]" />
                <span className="min-w-0 flex-1 truncate text-xs font-semibold">
                  {file.fileName}
                </span>
                <Download className="size-3.5 text-[var(--ink-faint)]" />
              </button>
            ))}
            {!receipt.files.length ? (
              <p className="rounded-xl bg-[var(--danger-soft)] px-3.5 py-3 text-xs text-[var(--danger)]">
                No finalized private file is bound to this receipt.
              </p>
            ) : null}
          </div>
        </section>
        ...
        {receipt.reviewStatus === "approved" ? (
          <section className="mt-7 border-t border-[var(--line)] pt-6">
            <SectionHeading
              ...
              detail={`All links on this page · latest ${referenceWindowSize} unlinked options`}
            />
            <div className="mt-3 space-y-4">
              <div>
                ...
                {linkedExpenses.map((expense) => (
                  <div
                    key={expense.id}
                    className="mt-2 flex items-center gap-2 rounded-xl bg-[var(--canvas)] px-3 py-2.5"
                  >
                    <Link2 className="size-3.5 text-[var(--positive)]" />
                    ...
                    <Button
                      ...
                      disabled={busy}
                      onClick={() =>
                        void linkReference("expense", expense.id, null)
                      }
                    >
                ...
                <div className="mt-2 flex gap-2">
                  ...
                  <Button
                    ...
                    variant="secondary"
                    size="sm"
                    disabled={busy || !expenseTargetId}
                    onClick={() =>
                      void linkReference("expense", expenseTargetId, receipt.id)
                    }
                  >
                    Link
                  </Button>
              ...
              <div>
                <p className="text-xs font-semibold">Inventory deliveries</p>
                {linkedDeliveries.map((delivery) => (
                  ...
                  >
                    ...
                    <Button
                      ...
                      disabled={busy}
                      onClick={() =>
                        void linkReference("delivery", delivery.id, null)
                      }
                    >
                      <Unlink className="size-3.5" />
                      Unlink
                ...
                <div className="mt-2 flex gap-2">
                  <select
                    aria-label="Unlinked inventory delivery"
                  ...
                  <Button
                    ...
                    variant="secondary"
                    size="sm"
                    disabled={busy || !deliveryTargetId}
                    onClick={() =>
                      void linkReference(
                        "delivery",
                        deliveryTargetId,
        ...
        <section className="mt-7 border-t border-[var(--line)] pt-6">
          <SectionHeading
            ...
            detail="Every field remains human-reviewed"
          />
          {receipt.extractions.length ? (
            <div className="mt-3 grid gap-2 sm:grid-cols-2">
              {receipt.extractions.map((extraction) => (
                <div
            ...
            <div className="mt-3 flex items-start gap-3 rounded-xl bg-[var(--accent-soft)]/45 p-4 text-xs leading-4 text-[var(--accent-strong)]">
              ...
              <span>
                No extraction evidence is stored for this document. This release
                does not send receipt images to a live model provider.
              </span>
            </div>
          )}
        </section>
        ...
        {message ? (
          <p
            role="status"
            className="mt-5 rounded-xl bg-[var(--canvas)] px-3.5 py-3 text-xs"
          >
            {message}
          </p>
        ) : null}
        {!terminal ? (
          <div className="mt-6 flex flex-wrap justify-end gap-2">
            ...
            <Button
              type="button"
              variant="danger"
              disabled={busy || Boolean(unresolvedMatches.length)}
            ...
            <Button
              ...
              title={
                unresolvedMatches.length
                  ? "Resolve possible duplicates first"
                  : undefined
              }
              onClick={() => void review("approved")}
            >
              <Check className="size-4" /> Approve and lock
            </Button>
          </div>
        ) : (
          <div className="mt-6 flex items-center gap-2 rounded-xl bg-[var(--positive-soft)] px-3.5 py-3 text-xs text-[var(--positive)]">
            <ShieldCheck className="size-4" /> Terminal reviews and their
...
export function ConnectedReceiptsWorkspace({
  ...
  function searchReceipts() {
    ...
    router.replace(
      normalized
        ? `/receipts?q=${encodeURIComponent(normalized)}`
        : "/receipts",
    );
  }
      ...
      <PageFrame>
        <section className="mx-auto mt-[10svh] max-w-xl rounded-[24px] border border-[var(--line)] bg-[var(--paper-strong)] p-8 text-center">
          ...
          <p className="mt-2 text-xs leading-5 text-[var(--ink-faint)]">
            {model.status === "forbidden"
              ? "Receipt and invoice records are limited to authorized restaurant management."
              : "Tenant-scoped receipt records could not be loaded safely. Retry without uploading a duplicate."}
          </p>
        </section>
      </PageFrame>
    );
  ...
  const reviewCount = receipts.filter((receipt) =>
    ["pending", "in_review"].includes(receipt.reviewStatus),
  ).length;
    ...
    <PageFrame width="wide">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-1 text-[13px] text-[var(--ink-faint)]">
            Capture, verify, search, and safely retrieve location-scoped
            evidence.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <input
            ...
            onChange={(event) => {
              ...
              if (file) void upload(file, "camera");
      ...
      <section className="mt-5 grid grid-cols-2 divide-x divide-y divide-[var(--line)] border-y border-[var(--line)] sm:grid-cols-4 sm:divide-y-0">
        <Metric
          ...
          detail={`${model.totalCount} matching · page ${model.page} of ${model.totalPages}`}
        />
        <Metric
          label="Page review"
          value={String(reviewCount)}
          detail="Pending or in review on this page"
        />
        <Metric
          label="Page total"
          value={formatMoney(totalCents, model.currencyCode)}
          detail="Loaded page only"
        />
        <Metric
          label="Page duplicates"
          value={String(
            receipts.reduce((sum, receipt) => sum + receipt.duplicateCount, 0),
          )}
          detail="Unresolved on this page"
        />
      </section>
      ...
      {message ? (
        <div
          role="status"
          className="mt-5 flex items-center gap-3 rounded-xl bg-[var(--accent-soft)]/45 px-4 py-3 text-xs text-[var(--accent-strong)]"
        >
      ...
      <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        >
          <label className="relative min-w-0 flex-1">
            <span className="sr-only">Search receipts and OCR text</span>
            <Search className="absolute top-1/2 left-3.5 size-4 -translate-y-1/2 text-[var(--ink-faint)]" />
            <input
              type="search"
              value={query}
              maxLength={120}
      ...
      {initialSearch ? (
        <div className="mt-3 flex items-center justify-between gap-3 text-xs text-[var(--ink-faint)]">
          <span>
            Showing tenant-scoped matches for “{initialSearch}”, including
            indexed OCR text.
          </span>
          <button
            className="focus-ring shrink-0 rounded-lg px-2 py-1 font-semibold text-[var(--accent-strong)]"
            onClick={() => {
              setQuery("");
      ...
      <section className="mt-5 overflow-hidden rounded-[20px] border border-[var(--line)] bg-[var(--paper-strong)]">
        ...
        {filtered.map((receipt) => (
          <button
            ...
            className="focus-ring grid w-full grid-cols-[1fr_auto] items-center gap-3 border-t border-[var(--line)] px-4 py-4 text-left first:border-t-0 sm:grid-cols-[1.2fr_.8fr_.7fr_.65fr_auto]"
          >
            <span className="min-w-0">
              ...
              <span className="mt-1 block text-xs text-[var(--ink-faint)]">
                {receipt.source} · {receipt.files.length} private file
                {receipt.files.length === 1 ? "" : "s"}
                {receipt.duplicateCount
                  ? ` · ${receipt.duplicateCount} unresolved match`
                  : ""}
              </span>
            </span>
            ...
            <span className="numeric text-xs font-semibold">
              {receipt.totalCents == null
                ? "—"
                : formatMoney(receipt.totalCents, model.currencyCode)}
            </span>
            <span className="hidden sm:block">
              <StatusPill tone={statusTone[receipt.reviewStatus] ?? "neutral"}>
                {receipt.reviewStatus.replaceAll("_", " ")}
              </StatusPill>
            </span>
            <FileSearch className="size-4 text-[var(--ink-faint)]" />
          </button>
        ))}
        {!filtered.length ? (
          <div className="px-6 py-16 text-center">
            <ReceiptText className="mx-auto size-6 text-[var(--ink-faint)]" />
            <p className="mt-3 text-xs font-semibold">
              No receipts match on this page
            </p>
            <p className="mt-1 text-xs text-[var(--ink-faint)]">
              Change the page, search, or local status filter.
            </p>
          </div>
        ) : null}
      </section>
      ...
      >
        <p className="text-xs text-[var(--ink-faint)]">
          Rows {(model.page - 1) * model.pageSize + (receipts.length ? 1 : 0)}–
          {(model.page - 1) * model.pageSize + receipts.length} of{" "}
          {model.totalCount} matching documents
        </p>
        <div className="flex gap-2">
          <Button
            variant="secondary"
            ...
            onClick={() => router.replace(pageHref(model.page - 1))}
          >
            Previous
          </Button>
          <Button
      ...
      {selected ? (
        <ReceiptInspector
          ...
          referenceWindowSize={model.referenceWindowSize}
