      ...
      <section ref={dialogRef} tabIndex={-1} aria-labelledby="receipt-review-title" aria-modal="true" role="dialog" className="ml-auto min-h-full w-full max-w-2xl rounded-[26px] bg-[var(--paper-strong)] p-5 shadow-[var(--shadow-float)] sm:p-8">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-xs text-[var(--ink-faint)]">Uploaded {new Date(receipt.createdAt).toLocaleString()}</p>
          </div>
        ...
        <form ref={formRef} className="mt-7" onSubmit={(event) => { event.preventDefault(); void review("in_review"); }}>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <label><span className="mb-1.5 block text-xs font-semibold">Document number</span><input name="documentNumber" defaultValue={receipt.documentNumber ?? ""} disabled={terminal} className="h-11 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 text-xs" /></label>
            <label><span className="mb-1.5 block text-xs font-semibold">Document date</span><input name="documentDate" type="date" defaultValue={receipt.documentDate ?? ""} disabled={terminal} className="h-11 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 text-xs" /></label>
            <label><span className="mb-1.5 block text-xs font-semibold">Total</span><input name="total" type="number" min="0" step="0.01" defaultValue={receipt.totalCents == null ? "" : (receipt.totalCents / 100).toFixed(2)} disabled={terminal} className="numeric h-11 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 text-xs" /></label>
            <label><span className="mb-1.5 block text-xs font-semibold">Tax</span><input name="tax" type="number" min="0" step="0.01" defaultValue={receipt.taxCents == null ? "" : (receipt.taxCents / 100).toFixed(2)} disabled={terminal} className="numeric h-11 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 text-xs" /></label>
            <label className="sm:col-span-2"><span className="mb-1.5 block text-xs font-semibold">Review notes</span><textarea name="notes" rows={3} defaultValue={receipt.notes ?? ""} disabled={terminal} className="w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] p-3 text-xs" /></label>
          </div>
          ...
          <section className="mt-7 border-t border-[var(--line)] pt-6">
            ...
            <div className="mt-3 space-y-2">
              {receipt.duplicateMatches.map((match) => <div key={match.id} className="rounded-xl bg-[var(--canvas)] p-3.5"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><p className="flex items-center gap-2 text-xs font-semibold"><GitCompare className="size-3.5 text-[var(--warning)]" />Receipt {match.possibleDuplicateId.slice(0, 8)}</p><p className="numeric mt-1 text-xs text-[var(--ink-faint)]">{Math.round(match.score * 100)}% match · {match.reasons.length ? match.reasons.map((reason) => reason.replaceAll("_", " ")).join(", ") : "No machine reason recorded"}</p></div>{match.resolution ? <StatusPill tone={match.resolution === "duplicate" ? "warning" : "positive"}>{match.resolution.replaceAll("_", " ")}</StatusPill> : <div className="flex gap-2"><Button type="button" variant="quiet" size="sm" disabled={busy} onClick={() => void resolveDuplicate(match.id, "not_duplicate")}>Separate</Button><Button type="button" variant="secondary" size="sm" disabled={busy} onClick={() => void resolveDuplicate(match.id, "duplicate")}>Duplicate</Button></div>}</div></div>)}
              {!receipt.duplicateMatches.length ? <p className="rounded-xl bg-[var(--positive-soft)] px-3.5 py-3 text-xs text-[var(--positive)]">No possible duplicate evidence is recorded for this document.</p> : null}
            </div>
          </section>
          ...
          <section className="mt-7 border-t border-[var(--line)] pt-6">
            ...
            <div className="mt-3 space-y-2">
              {receipt.files.map((file) => <button key={file.id} type="button" onClick={() => void openFile(file)} className="focus-ring flex w-full items-center gap-3 rounded-xl bg-[var(--canvas)] px-3.5 py-3 text-left"><ReceiptText className="size-4 text-[var(--accent-strong)]" /><span className="min-w-0 flex-1 truncate text-xs font-semibold">{file.fileName}</span><Download className="size-3.5 text-[var(--ink-faint)]" /></button>)}
              {!receipt.files.length ? <p className="rounded-xl bg-[var(--danger-soft)] px-3.5 py-3 text-xs text-[var(--danger)]">No finalized private file is bound to this receipt.</p> : null}
            </div>
          </section>

          {receipt.reviewStatus === "approved" ? <section className="mt-7 border-t border-[var(--line)] pt-6"><SectionHeading title="Accounting links" detail={`All links on this page · latest ${referenceWindowSize} unlinked options`} /><div className="mt-3 space-y-4"><div><p className="text-xs font-semibold">Expenses</p>{linkedExpenses.map((expense) => <div key={expense.id} className="mt-2 flex items-center gap-2 rounded-xl bg-[var(--canvas)] px-3 py-2.5"><Link2 className="size-3.5 text-[var(--positive)]" /><span className="min-w-0 flex-1 truncate text-xs">{expense.label}</span><Button type="button" variant="quiet" size="sm" disabled={busy} onClick={() => void linkReference("expense", expense.id, null)}><Unlink className="size-3.5" />Unlink</Button></div>)}<div className="mt-2 flex gap-2"><select aria-label="Unlinked expense" value={expenseTargetId} onChange={(event) => setExpenseTargetId(event.target.value)} className="h-10 min-w-0 flex-1 rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 text-xs"><option value="">Choose a recent unlinked expense</option>{availableExpenses.map((expense) => <option key={expense.id} value={expense.id}>{expense.label}</option>)}</select><Button type="button" variant="secondary" size="sm" disabled={busy || !expenseTargetId} onClick={() => void linkReference("expense", expenseTargetId, receipt.id)}>Link</Button></div></div><div><p className="text-xs font-semibold">Inventory deliveries</p>{linkedDeliveries.map((delivery) => <div key={delivery.id} className="mt-2 flex items-center gap-2 rounded-xl bg-[var(--canvas)] px-3 py-2.5"><Link2 className="size-3.5 text-[var(--positive)]" /><span className="min-w-0 flex-1 truncate text-xs">{delivery.label}</span><Button type="button" variant="quiet" size="sm" disabled={busy} onClick={() => void linkReference("delivery", delivery.id, null)}><Unlink className="size-3.5" />Unlink</Button></div>)}<div className="mt-2 flex gap-2"><select aria-label="Unlinked inventory delivery" value={deliveryTargetId} onChange={(event) => setDeliveryTargetId(event.target.value)} className="h-10 min-w-0 flex-1 rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 text-xs"><option value="">Choose a recent unlinked delivery</option>{availableDeliveries.map((delivery) => <option key={delivery.id} value={delivery.id}>{delivery.label}</option>)}</select><Button type="button" variant="secondary" size="sm" disabled={busy || !deliveryTargetId} onClick={() => void linkReference("delivery", deliveryTargetId, receipt.id)}>Link</Button></div></div></div></section> : null}

          <section className="mt-7 border-t border-[var(--line)] pt-6">
            <SectionHeading title="Extraction evidence" detail="Every field remains human-reviewed" />
            {receipt.extractions.length ? <div className="mt-3 grid gap-2 sm:grid-cols-2">{receipt.extractions.map((extraction) => <div key={extraction.fieldName} className="rounded-xl bg-[var(--canvas)] p-3"><p className="text-xs font-semibold text-[var(--ink-faint)]">{extraction.fieldName}</p><p className="mt-1 truncate text-xs font-semibold">{extraction.value}</p><p className="numeric mt-1 text-xs text-[var(--ink-faint)]">{extraction.confidence == null ? "Confidence unavailable" : `${Math.round(extraction.confidence * 100)}% confidence`}</p></div>)}</div> : <div className="mt-3 flex items-start gap-3 rounded-xl bg-[var(--accent-soft)]/45 p-4 text-xs leading-4 text-[var(--accent-strong)]"><Sparkles className="mt-0.5 size-4 shrink-0" /><span>No extraction evidence is stored for this document. This release does not send receipt images to a live model provider.</span></div>}
          </section>
