...
import { formatLocalDateTime } from "@/data/read-models/local-time";
...
export function LiveReportsWorkspace({
  ...
  const authorizedCatalog = catalog.filter((entry) =>
    canAccessReportKind(workspace, entry.kind),
  );
  const group = authorizedCatalog.find((item) => item.kind === report.kind)?.group ?? "Operations";
  ...
  const reportTimeZone = workspace.activeLocation.timeZone ?? "America/New_York";

  return (
    <PageFrame width="full" className="max-w-[1700px]">
      <header className="flex flex-col justify-between gap-5 xl:flex-row xl:items-end">
        <div><div className="flex items-center gap-2"><StatusPill tone="positive" dot>Source-backed</StatusPill><span className="text-xs text-[var(--ink-faint)]">Generated {formatLocalDateTime(model.generatedAt, reportTimeZone)}</span></div><h2 className="mt-3 text-2xl font-medium tracking-[-0.045em]">Reports</h2><p className="mt-1 text-[13px] text-[var(--ink-faint)]">Tenant-scoped operational evidence for {workspace.organization.name}.</p></div>
      ...
      <div className="mt-6 lg:hidden"><label><span className="mb-1.5 block text-xs font-semibold text-[var(--ink-faint)]">Report view</span><select value={report.kind} onChange={(event) => router.push(reportUrl(event.target.value as ReportKind, model))} className="h-11 w-full rounded-xl border border-[var(--line)] bg-[var(--paper-strong)] px-3 text-xs font-semibold outline-none">{authorizedCatalog.map((item) => <option key={item.kind} value={item.kind}>{item.group} · {item.label}</option>)}</select></label></div>
      <Tabs id="live-report-views" label="Report views" className="mt-6 hidden border-y lg:flex" size="large" items={authorizedCatalog.map((item) => ({ value: item.kind, label: item.label }))} value={report.kind} onValueChange={(kind) => router.push(reportUrl(kind, model))} />
