...
function attendanceReport(filters: ReportFilters): ReportView {
  ...
  return {
    ...
    metrics: [
      ...
      { label: "Open", value: String(records.filter((record) => record.status === "open").length), detail: "Clock-out pending" },
    ...
    columns: [
      ...
      { key: "location", label: "Location" },
      { key: "date", label: "Date" },
      { key: "clockIn", label: "Clock in", align: "right" },
      { key: "clockOut", label: "Clock out", align: "right" },
