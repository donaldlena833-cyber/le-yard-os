import { buildReportCsv } from "@/components/reports/report-csv";
...
export async function GET(request: Request) {
  ...
  if (runtime.mode === "connected") {
    ...
    try {
      audit = await beginInlineReportExport(report, "csv");
      const csv = buildReportCsv(report.model.view, report.model.filters);
      ...
      const filename = `le-yard-${report.model.view.kind}-${report.model.filters.startsOn}-${report.model.filters.endsOn}.csv`;
      ...
      if (audit) {
        try {
          await finalizeInlineReportExport(audit, {
            status: "failed",
            errorMessage: "Inline CSV generation failed.",
          });
        } catch {
