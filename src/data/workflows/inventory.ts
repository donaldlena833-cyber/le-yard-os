...
async function requireInventoryCapability(
  ...
  if (error) {
    throwDatabaseError(
      error,
      "Your inventory capability could not be verified.",
    );
  }
  assertCondition(
    data === true,
...
export async function configureInventoryCatalog(
  ...
  const operationalCapabilityByCommand = {
    ...
    Record<ConfigureInventoryCatalogInput["command"], OperationalCapability>
  >;
  const operationalCapability =
    operationalCapabilityByCommand[
      input.command as keyof typeof operationalCapabilityByCommand
    ];
  const managerOperationalEdit =
    operationalRole &&
    (operationalCapability !== undefined ||
  ...
  if (managerRecipeEdit || managerOperationalEdit) {
    const requiredCapabilities: OperationalCapability[] =
      input.command === "recipe.save"
        ? ["recipe.manage"]
        : input.command === "vendor_item.save"
    ...
    const capabilityError = capabilityResults.find(
      (result) => result.error,
    )?.error;
    if (capabilityError) {
      throwDatabaseError(
  ...
  if (managerRecipeEdit) {
    ...
    if (result.error)
      throwDatabaseError(result.error, "The recipe could not be saved.");
    assertCondition(
    ...
    const managerResult = result.data as {
      id: unknown;
      command: unknown;
      replayed?: unknown;
    };
  ...
  const payload = Object.fromEntries(
    Object.entries(input).filter(
      ([key]) =>
        key !== "requestId" &&
        key !== "workspaceLocationId" &&
        key !== "command",
    ),
  );
  const kitchenFoundationEdit =
  ...
  if (error)
    throwDatabaseError(error, "The inventory setup change could not be saved.");
  assertCondition(
    typeof data === "object" &&
      data !== null &&
      "id" in data &&
...
export async function recordInventoryItemCost(
  ...
  if (!elevated) {
    ...
    if (error)
      throwDatabaseError(
        error,
        "Your inventory pricing capability could not be verified.",
      );
    assertCondition(
      data === true,
      "forbidden",
      "Inventory price management is not assigned at this location.",
    );
  }
  ...
  if (error)
    throwDatabaseError(error, "The inventory unit cost could not be saved.");
  assertCondition(
...
async function replayExistingInventoryCount(
  ...
  const { data: existing, error: existingError } = await context.supabase
    ...
    .select(
      "id, organization_id, location_id, status, count_type, counted_by, counted_at, notes",
    )
  ...
  if (existingError) {
    throwDatabaseError(
      existingError,
      "The inventory submission could not be checked.",
    );
  }
  ...
  if (lineError) {
    throwDatabaseError(
      lineError,
      "The existing inventory count could not be verified.",
    );
  }
  const requested = new Map(
    input.lines.map((line) => [line.inventoryItemId, line]),
  );
  assertCondition(
...
async function buildAuthoritativeCountLines(
  ...
  const { data: items, error: itemError } = await context.supabase
    ...
    .order("id");
  if (itemError)
    throwDatabaseError(itemError, "Inventory items could not be verified.");
  assertCondition(
    items?.length,
    "conflict",
    "There are no active tracked items to count.",
  );

  const submittedByItem = new Map<
    string,
    SubmitInventoryCountInput["lines"][number]
  >();
  ...
  const [
    { data: onHandRows, error: onHandError },
    ...
    context.supabase
      ...
      .select(
        "inventory_item_id, unit_id, price_quantity, unit_price_cents, effective_at",
      )
      .eq("organization_id", organizationId)
      .in("inventory_item_id", itemIds)
  ...
  if (onHandError)
    throwDatabaseError(
      onHandError,
      "Current on-hand quantities could not be verified.",
    );
  if (priceError)
    throwDatabaseError(
      priceError,
      "Current inventory costs could not be verified.",
    );
  ...
  return items.map((item) => {
    ...
    return {
      ...
      unit_cost_cents:
        latestPrice.get(`${item.id}:${item.base_unit_id}`) ?? null,
...
export async function submitInventoryCount(
  ...
  if (error)
    throwDatabaseError(error, "The inventory count could not be submitted.");
  const count = assertFound(
    data,
    "The submitted inventory count was not returned.",
  );
...
export async function approveInventoryCount(
  ...
  if (countError)
    throwDatabaseError(
      countError,
      "The inventory count could not be verified.",
    );
  const existing = assertFound(
    countRecord,
    "The inventory count was not found.",
  );
  ...
  if (error)
    throwDatabaseError(
      error,
      "The inventory count review could not be recorded.",
    );
  const count = assertFound(
    data,
    "The reviewed inventory count was not returned.",
  );
  return {
    id: count.id as string,
    status: count.status as string,
...
export async function createPurchaseOrder(
  ...
  if (error)
    throwDatabaseError(error, "The purchase order could not be created.");
  const order = assertFound(
    data,
    "The created purchase order was not returned.",
  );
  return {
    id: String(order.id),
...
export async function reviewPurchaseOrder(
  ...
  const { data: record, error } = await context.supabase
    ...
    .maybeSingle();
  if (error)
    throwDatabaseError(error, "The purchase order could not be verified.");
  const existing = assertFound(record, "The purchase order was not found.");
  await requireInventoryCapability(
    context,
    existing.location_id,
  ...
  assertCondition(
    ...
    "conflict",
    "A different manager must review this purchase order.",
  );
  const { data, error: reviewError } = await context.supabase.rpc(
  ...
  if (reviewError)
    throwDatabaseError(
      reviewError,
      "The purchase-order review could not be recorded.",
    );
  const order = assertFound(
    data,
    "The reviewed purchase order was not returned.",
  );
  return {
    id: String(order.id),
    status: String(order.status),
  ...
  const { data, error } = await context.supabase.rpc(
    "receive_inventory_delivery_with_exceptions",
    {
      ...
      p_lines: input.lines.map((line) => ({
        ...
        exception_kind: line.exceptionKind,
        exception_note: normalizedNote(line.exceptionNote),
      })),
    },
  );
  if (error)
    throwDatabaseError(error, "The inventory delivery could not be received.");
  assertCondition(
    typeof data === "object" && data !== null && !Array.isArray(data),
    "database",
    "The received delivery was not returned.",
  );
  const delivery = data as { id?: unknown; status?: unknown; exceptionCount?: unknown };
  assertCondition(delivery.id, "database", "The received delivery id was not returned.");
  return {
    id: String(delivery.id),
    status: String(delivery.status),
    exceptionCount: Number(delivery.exceptionCount ?? 0),
  };
}

export async function reviewDeliveryExceptions(
  context: WorkflowContext,
  input: ReviewDeliveryExceptionsInput,
) {
  const { data: record, error } = await context.supabase
    .from("delivery_receiving_batches")
    .select("delivery_id, location_id")
    .eq("delivery_id", input.deliveryId)
    .maybeSingle();
  if (error) throwDatabaseError(error, "The delivery exceptions could not be verified.");
  const existing = assertFound(record, "The delivery exception review was not found.");
  await requireInventoryCapability(context, existing.location_id, "inventory.receive");
  const { data, error: reviewError } = await context.supabase.rpc(
    "review_delivery_receiving_exceptions",
    {
      p_request_id: input.requestId,
      p_posting_request_id: input.postingRequestId,
      p_delivery_id: input.deliveryId,
      p_approve: input.approve,
      p_note: normalizedNote(input.note),
    },
  );
  if (reviewError) throwDatabaseError(reviewError, "The delivery exception review could not be saved.");
  const batch = assertFound(data, "The reviewed delivery exception batch was not returned.");
  return { deliveryId: String(batch.delivery_id), status: String(batch.status) };
}
...
export async function submitWasteRecord(
  ...
  if (error)
    throwDatabaseError(error, "The waste record could not be submitted.");
  const waste = assertFound(
    data,
    "The submitted waste record was not returned.",
  );
  return {
    id: String(waste.id),
    status: String(waste.status),
    estimatedCostCents:
      waste.estimated_cost_cents == null
        ? null
...
export async function reviewWasteRecord(
  ...
  const { data: record, error } = await context.supabase
    ...
    .maybeSingle();
  if (error)
    throwDatabaseError(error, "The waste record could not be verified.");
  ...
  if (reviewError)
    throwDatabaseError(reviewError, "The waste review could not be recorded.");
  const waste = assertFound(
    data,
    "The reviewed waste record was not returned.",
  );
  return {
    id: String(waste.id),
    status: String(waste.status),
...
export async function createInventoryTransfer(
  ...
  if (error)
    throwDatabaseError(error, "The inventory transfer could not be submitted.");
  const transfer = assertFound(
    data,
    "The submitted inventory transfer was not returned.",
  );
...
export async function reviewInventoryTransfer(
  ...
  if (reviewError)
    throwDatabaseError(
      reviewError,
      "The inventory transfer review could not be recorded.",
    );
  const transfer = assertFound(
    data,
    "The reviewed inventory transfer was not returned.",
  );
  return {
