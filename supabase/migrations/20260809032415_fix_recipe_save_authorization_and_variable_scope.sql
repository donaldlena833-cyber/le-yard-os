-- Repair the frozen Manager recipe command without rewriting migration 024.
-- The original PL/pgSQL variable `organization_id` collided with table-column
-- names during ingredient insertion. This replacement gives the actor-derived
-- organization a distinct name and moves recipe.manage enforcement into the
-- database boundary so direct RPC calls cannot bypass the server workflow.
...
begin
  ...
  if not public.has_capability(
    ...
    raise exception 'Recipe management capability is required' using errcode = '42501';
  end if;
  if p_request_id is null or target_id is null or clean_name is null
    ...
    or (
      p_menu_price_cents is not null
      and (p_menu_price_cents < 0 or p_menu_price_cents > 9000000000000000)
    )
  ...
  if not exists (
    ...
    where unit.id = p_yield_unit_id
      and unit.organization_id = resolved_organization_id
      and unit.is_active
  ) then
  ...
  if exists (
    ...
    where jsonb_typeof(ingredient) <> 'object'
      or not (
        ingredient ?& array[
          'inventoryItemId', 'unitId', 'quantity', 'wasteFactor'
        ]
      )
    ...
    raise exception 'Recipe ingredients are invalid or duplicated'
      using errcode = '22023';
  end if;
  if (
    select count(*)
  ...
  for update;
  if existing_recipe.id is not null
    and existing_recipe.organization_id <> resolved_organization_id then
...
revoke all on function public.save_manager_recipe(
  uuid, uuid, uuid, text, numeric, uuid, bigint, boolean, jsonb
) from public, anon, authenticated;
grant execute on function public.save_manager_recipe(
  uuid, uuid, uuid, text, numeric, uuid, bigint, boolean, jsonb
) to authenticated;

comment on function public.save_manager_recipe(
  uuid, uuid, uuid, text, numeric, uuid, bigint, boolean, jsonb
) is
  'Actor-derived, idempotent, capability-enforced recipe version command for an assigned Manager/Chef location.';
