...
test("searches OCR text and verifies a receipt", async ({ page }) => {
  ...
  await page
    ...
    .fill("Harbor");
  await page.getByRole("button", { name: /Harbor Produce — Demo/ }).click();

  const receiptPanel = page.locator("aside").filter({
    has: page.getByRole("heading", { name: "Harbor Produce — Demo", exact: true }),
  });
  await expect(receiptPanel).toBeVisible();
  await receiptPanel.getByRole("button", { name: "Verify fields", exact: true }).click();
  await expect(receiptPanel.getByRole("button", { name: "Verified", exact: true })).toBeDisabled();
...
test("updates and submits a live inventory count", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Live count", exact: true })).toBeVisible();
...
test("searches CRM allergy data and opens the matching guest", async ({ page }) => {
  ...
  const guestPanel = page.locator("aside").filter({ hasText: "Nora Example" });
  await expect(guestPanel).toBeVisible();
  await expect(guestPanel.getByRole("heading", { name: "Contact & consent" })).toBeVisible();
  await expect(guestPanel.getByText("nora@example.invalid", { exact: true })).toBeVisible();
  await expect(guestPanel.getByText("tree nuts", { exact: true })).toBeVisible();
  await expect(guestPanel.getByRole("heading", { name: "Visit history" })).toBeVisible();
  await expectNoViewportOverflow(page);
});
