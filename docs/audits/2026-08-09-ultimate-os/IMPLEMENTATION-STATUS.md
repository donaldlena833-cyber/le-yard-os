...
| Gate 2 — resilient operations  | **Substantial repository slice; connected proof remains** | Scoped reservation RPCs and immutable replay evidence; typed `ServiceDaySnapshot`; separate snapshot-read and provider-sync freshness; floor-now/future-inventory projections; capability-aware reservation UI; atomic schedule/template commands with publish serialization; exact sensitive Today and Inventory commands; materialized timezone-aware service shifts with native PostgreSQL serialization; responsive reservation floor/book; privacy-preserving chat child-delete convergence; shared coalesced, reconnect-aware, ID-only Realtime invalidation with explicit stale states. | Run connected Realtime publication/reconnect acceptance; add approved append-only offline outbox behavior; finish responsive alternatives across dense modules. |
...
Passed on the current repository worktree:

- Generated database contract (139 tables, 3 views, 259 public functions, 16 enums), typecheck, 127 unit files / 737 tests, every portable migration/policy suite, and the production build on the final merged follow-up worktree.
- Reservation database lifecycle/security PGlite suite, materialized service-shift lifecycle suite, Income operating-snapshot suite, exact Inventory capability suite, Realtime publication contract, and the 259-function grant audit.
...
Not yet passed or externally available:

- Reservation, schedule, service-shift, provider-lease, and message-revocation concurrency contracts have run against disposable local PostgreSQL 17 clusters where documented. Their GitHub jobs must remain green after every push; none replaces connected Supabase acceptance.
...
1. Run the connected role/location and Realtime matrix in an isolated production-like Supabase environment.
2. Prove Auth, RLS, cron, monitoring, managed backup/PITR, private Storage recovery, and secret rotation. The repository-controlled disposable PostgreSQL restore drill is a separate local/CI proof.
