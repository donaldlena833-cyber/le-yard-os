...
| Gate 2 — resilient operations  | **Substantial repository slice; lifecycle work remains**  | Scoped reservation RPCs and replay evidence; typed `ServiceDaySnapshot`; separate snapshot-read and provider-sync freshness; floor-now/future-inventory projections; capability-aware reservation UI; atomic schedule/template commands with publish serialization; exact sensitive Today reads; materialized timezone-aware service shifts with permission-aware exception authoring; responsive reservation floor/book; shared coalesced, reconnect-aware, ID-only Realtime invalidation with explicit stale states. | Run native exception concurrency and connected Realtime publication/reconnect acceptance; consolidate remaining multi-record workflows; add approved append-only offline outbox behavior; finish responsive alternatives across dense modules.                                                   |
